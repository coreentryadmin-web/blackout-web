...
import { createAuditClient, resolveAuditDbUrl, isPrivateDbUnreachableError, isStaleAuditDbAuthError } from "./pg-audit.mjs";
...
async function postgresItems() {
  ...
  for (const key of zeroRuns) {
    add("P0", "cron", `cron:${key}:never-fired`, `Cron never fired: ${key}`, "Zero rows in cron_job_runs — ECS cron service or config-as-code likely missing.");
  }
