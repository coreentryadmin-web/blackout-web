...
import { socketProbeAttemptVerdict, socketProbeFinalFailure, isSocketHealthNonTradingSkip } from "./lib/rth-socket-probe.mjs";
  ...
  if (force || inRthOpenWindow(now)) {
      ...
      for (let attempt = 0; attempt < 3 && !socketProbeOk; attempt++) {
        try {
          ...
          if (isSocketHealthNonTradingSkip(body)) {
            ok(`options-socket: ${typeof body.reason === "string" ? body.reason : "non-trading day skip"}`);
            socketProbeOk = true;
            break;
          }
    ...
    try {
      ...
      if (process.env.GITHUB_ACTIONS === "true") {
        console.log("  ⚠ production uw-socket log check skipped in GitHub Actions");
      } else {
        console.log("  ⚠ Could not read production logs for uw-socket");
      }
    }
