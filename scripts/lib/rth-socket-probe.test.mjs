...
test("isSocketHealthNonTradingSkip: holiday skip payload passes", () => {
  assert.equal(
    isSocketHealthNonTradingSkip({
      ok: true,
      skipped: true,
  ...
  assert.equal(isSocketHealthNonTradingSkip({ ok: true, skipped: false }), false);
});

test("probeOptionsSocketWithRetries: non-trading-day skip passes without websockets", async () => {
  let calls = 0;
  const result = await probeOptionsSocketWithRetries({
    afterOpen930: true,
    fetchSocketHealth: async () => {
