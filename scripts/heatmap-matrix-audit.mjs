...
/**
 * Cumulative gamma flip — matches production `cumulativeGammaFlip` and
 * `deriveCumulativeGammaFlip` in heatmap-verifier.ts (NOT per-strike crossings).
 */
...
async function auditTicker(ticker) {
  ...
  const nearExpiries = resolveNearTermExpiriesForAudit(hm);
  let totalFlags = 0;
  let totalChecks = 0;
