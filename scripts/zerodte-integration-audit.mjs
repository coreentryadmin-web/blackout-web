...
async function auditCrossToolLive() {
  ...
  if (boot.status === 200 && boot.json.error !== "coming_soon") {
    ...
    // Bootstrap and gex-positioning read different cache lanes; allow ~1.5 pt refresh skew.
    if (Number.isFinite(bootSpot) && Number.isFinite(gexSpot) && spotDelta(bootSpot, gexSpot) > 1.5) {
  ...
  if (Number.isFinite(liveSpot) && Number.isFinite(gexSpot) && spotDelta(liveSpot, gexSpot) > 1.5) {
