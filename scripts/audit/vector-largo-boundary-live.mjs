...
async function main() {
  ...
  for (const ticker of TICKERS) {
    ...
    const wh = await fetchAuditJson(BASE, `/api/market/vector/wall-history?ticker=${ticker}`);
    const samples = Array.isArray(wh.json?.samples) ? wh.json.samples.length
      : Array.isArray(wh.json?.history) ? wh.json.history.length : null;
    row.wall_history = { http: wh.status, samples };
    ...
    log(`  wall-history   HTTP ${row.wall_history.http}  samples = ${row.wall_history.samples}`);
