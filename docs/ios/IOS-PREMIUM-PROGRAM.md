...
- [ ] **N-2b APNs pipeline — native side** — SwiftUI `PushRegistrationService` in `apps/blackout-ios-native/BlackOut/Common/` that requests permission, calls `UIApplication.registerForRemoteNotifications`, captures the APNs token, and POSTs to `/api/push/native/register`. Hide the inert web-push toggle in the WKWebView shell.
