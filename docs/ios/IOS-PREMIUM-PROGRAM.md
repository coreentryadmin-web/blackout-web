...
- [x] **Command tab v2 — Product Pulse** (2026-07-27) — Master prompt §5. Six `ProductPulseCard`s on Command, each with real per-desk one-line intelligence derived from the shared regime snapshot. SPX shows spot-vs-flip distance in pts. Thermal shows call/put wall prices. Vector links SPX flip → cross-ticker. Helix / Largo / Night Hawk state their scope honestly until per-desk feeds land. Each card is a NavigationLink into `ProductDetailView` — same destination the Intelligence tab uses (no duplicated screens).
- [x] **Command tab v3 partial — What Changed timeline** (2026-07-27) — new `?history=N` mode on `/api/market/regime` (shared shaper `shapeSnapshotForClient` so single-latest and history modes can't drift). Native `MarketRegimeRepository.history(limit:)` binds to it. `WhatChangedDiff` pure function computes regime transitions (composite / GEX / vol / trend / flow / VWAP) newest-first, capped, empty when regime is stable. `WhatChangedCard` renders the diff with strikethrough-from → arrow → to, freshness label per row. Auto-refresh polls latest at 30s + history every 4th tick. Preserves previous history on transient error. 7 diff tests. **P0 regression harness `scripts/ios/ios-p0-regression.mjs`** (`npm run validate:ios-p0`) — fast browser-less gate that verifies the 4 submission blockers can't regress silently (P0-1 /privacy public+content, P0-2 iOS UA has no pricing tokens, P0-3 desktop UA still shows pricing + CF cache not HIT on iOS, P0-4 Whop not in allowNavigation). Live-tested against prod today: P0-3 CF change verified DYNAMIC, P0-1/P0-2 red (branch not merged) — the exact regression detection the harness is for. Also **fixed a bug in the invented Swift MarketRegime shape** — the prior version referenced snake_case fields (`flip_level`, `call_wall`) that the endpoint does NOT return; now decodes the ACTUAL camelCase response (`regime`/`gexRegime`/`volRegime`/`trendRegime`/`flowRegime`/`playbook`/`aboveVwap`/`stale`/`marketOpen`).
- [ ] **Command tab v4** — Top intelligence brief, Active opportunities, wired to real /api/plays.
- [ ] **Signals tab v2** — Real `/api/signals` feed with lifecycle transitions + push-triggered updates.
- [x] **Watchlist tab v2 — server sync** (2026-07-27) — new `GET/PUT /api/user/watchlist` endpoint (Clerk-auth, lazy `user_watchlists` table with `TEXT[]` ticker column so insertion order is preserved). Server sanitize reuses the web `normalizeTicker` for byte-for-byte agreement (no drift). Native `WatchlistSyncService` (protocol-injected `WatchlistSyncing`) reads via GET, writes via PUT; the store hydrates on first `.task` in `BlackOutApp` and pushes on every persist. Conflict policy: last-write-wins. Local UserDefaults remains the render source so the app is instant + offline-safe. Server-echo rewrites local when it differs (respects server normalize/dedupe/cap). 6 unit tests: hydrate sets didInitialSync, remote overrides local on first pull, no-op when remote matches, keeps local on network failure, push after add, echo rewrite. 5 server tests on `sanitize` (normalize, dedupe order-preserving, drop non-strings, cap at MAX_WATCHLIST).
- [ ] **Watchlist tab v3** — per-ticker detail + alert builder.
