...
- [ ] **N-2c Hide inert web-push toggle in WKWebView shell** — the existing web-push toggle shows "unsupported" inside the Capacitor WebView (VAPID doesn't work there); gate it out via `isIosAppShell()` so members don't see the dead affordance.
