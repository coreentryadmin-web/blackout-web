...
| **BREAKOUT** (`screenBreakoutMovers`, `candidates.ts:440`) | A grouped-daily mover with price $5–400, volume ≥ 1M, intraday gain ≥ 5%, close-strength ≥ 0.5; top-6 by $-volume (`BREAKOUT_MAX_CANDIDATES=6`). RTH-only [9:30,15:00). | 🟡 flag-gated | **Emits ZERO unless `ZERODTE_WHOLE_MARKET` + `ZERODTE_SRC_BREAKOUT` are set** (`scan.ts:295`). On a quiet tape the 5%-gain filter can genuinely starve it — that's by design (no forced plays). The top-6 cap is leaky (see discovery-recall-probe / FINDINGS 2026-07-25) but does not block emission. |
