...
| **Evidence** | `npx tsc --noEmit -p .` clean. `node --import tsx --experimental-test-module-mocks --test` on the full `src/features/vector/**/*.test.ts` suite: 529/529 pass (includes the new `vector-bead-recorder-core.test.ts`, `vector-bead-record/route.test.ts`, and the pre-existing `vector-walls-warm.test.ts`, which needs the project's `--experimental-test-module-mocks` flag — a first pass without it crashed the whole file with a misleading `mock.module is not a function` error, confirmed as a missing-flag artifact by rerunning with the flag, same false-alarm class documented in this session's continuous-monitor notes). `npx eslint` on every touched file: 0 errors (3 pre-existing `react-hooks/exhaustive-deps` warnings on `VectorChart.tsx`, confirmed present on `origin/main` before this diff — not introduced here). `npx next build` — full production build succeeds. |
| **Blast radius** | New files: `src/lib/vector-bead-recorder-leader.ts`, `src/features/vector/lib/vector-bead-recorder-core.ts` (+test), `vector-bead-recorder-logic.ts`, `src/app/api/cron/vector-bead-record/route.ts` (+test), `scripts/audit/vector-bead-probe.mjs`, `railway.vector-bead-record.toml`. Modified: `vector-universe.ts` (extracted `recordVectorUniverseWallSample` as a shared single-ticker recorder used by both the 5s leader and the existing 5-min cron), `vector-walls-warm.ts` (bead-recording removed, cache-priming only), `vector-snapshot.ts` (client-side narrowed-horizon accumulator), `VectorChart.tsx` (the accumulator's `useEffect`), `(site)/vector/page.tsx` (SPY/QQQ default-0DTE), `process-role.ts`/`web-boot-warm.ts`/`init-data-sockets.ts` (boot wiring, mirrors `rth-warm-leader` exactly), `cron-registry.ts`/`cron-dispatch.ts` (new cron entry). No changes to 0DTE/Swing/Banger grading, ledger, or commit logic — Vector-only. |
| **Status** | BUILT, VERIFIED, PR opened (`fix/vector-bead-density-universe` → `main`) → CI → auto-merge per standing policy. Original mixed draft PR #1707 left untouched/unmerged (its Thermal Discord content needs its own separate PR before that branch can land). |
