...
- HIGH scoring: published conviction/score are Claude's SELF-GRADE (`claude-edition.ts:60-70`) — the 842-line deterministic scorer only selects the top-12 candidates; nothing pins `play.score` to the computed score.
...
- HIGH discovery: HELIX Postgres bonus re-scores the SAME UW tape already in flow_score, adds up to +38 direction-blind, bypassing the regime multiplier — the most plausible false-strong source.
...
- HIGH grounding: entry/target/stop levels are SOFT-flagged only, never dropped/corrected; the check is skipped entirely when no S/R+chain reference exists; grounding also skips values <0.5×spot (the exact corrupt-entry-range class). Strike+premium grounding, by contrast, is genuinely HARD and solid.
- HIGH confirm: missing intel/Polygon data → every play silently CONFIRMED ("All checks passed" on zero data); only the SPX INDEX gap is checked — a play whose own stock gaps through entry/stop still confirms.
- MEDIUM grading: outcomes ignore entry-range reachability (gap-through-entry counts as a win from an unfillable entry — overstates win rate); morning-confirm prior-close is off-by-one pre-open (inflates gap, can false-INVALIDATE); scorer's earnings-proximity −6 and analyst-PT nudge are dead code in the edition path; buyback triple-counts (+7); mono-source discovery (top-net-impact/OI-change fetched but unused); no sector-concentration cap; flow "streak" counts DB buckets not consecutive days; no sub-$1/warrant/leveraged-ETF filter; play-critic fails open at every step (advisory, not binding); Hawk-Intel explainer prose has zero post-generation grounding; no as-of timestamps in Claude prompts; zero-candidate recap-only treated as benign (indistinguishable from total UW outage — no alert).
