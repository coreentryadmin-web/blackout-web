...
## 🔴 HIGH — `gex_cross_validation`'s UW oracle compares ALL-expiries-combined walls against Polygon's deliberately NEAR-TERM-ONLY walls — a structural scope mismatch, not (only) a data-completeness bug
**Status:** CONFIRMED via independent multi-agent code trace (2026-07-01). Root-caused in response to a member-visible "UW oracle diverges 550pt from Polygon walls — treat levels as provisional until channels agree" banner on the live SPX Slayer matrix.
...
**UI recommendation:** do NOT remove the "UW oracle diverges" banner — it may still be catching genuine truncation-driven corruption some of the time, and removing it would hide real data-quality regressions from members. **Root fix:** re-scope `getUwStrikeLadder` (`gex-cross-validation.ts`) to sum only the same near-term expiry window Polygon uses for `strikeTotals`, using UW's existing per-expiry endpoints, so the comparison becomes apples-to-apples.
