...
| **Fix** | NOT YET APPLIED — scoped for a follow-up PR: replace `planFromLegacyLevels()`'s direct reuse of Legacy's published entry/stop/target with a call to the SAME `deriveSwingPlanLevels(direction, price, atrProxyFromCloses(closes))` organic Swing discovery already uses, grounding the promoted play's geometry in the underlying's own multi-day ATR rather than an overnight-thesis band. |
| **Status** | DOCUMENTED, NOT YET BUILT. Tracked as the next concrete Swing follow-up. |
