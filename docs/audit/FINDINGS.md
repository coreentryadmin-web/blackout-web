...
## 2026-08-04 — [Architecture research] Why does whole-market 0DTE scanning never produce a 100%+ winner? (multi-agent deep dive + live re-measurement)

**Trigger.** Operator: "out of the whole market we could not find a single 100% or a big winner play... analyze the entire architecture." Dispatched 5 parallel research agents across FLOW/BREAKOUT/PIN discovery, contract/strike selection, exit-engine ceiling, and gate/scoring funnel bias, then live-verified the sharpest claim (BREAKOUT_MAX_CANDIDATES recall) by re-running `discovery-recall-probe.mjs` against TODAY's session with the CURRENT cap, since the last run (2026-07-25) measured a stale cap (6) and stale rank basis ($-volume, since replaced by momentum-quality rank 2026-07-28).

**Findings (code-verified, file:line, by agent):**
1. **BREAKOUT** (`breakout-discovery.ts`) is genuinely whole-market (~11.7k tickers/day via Polygon grouped-daily). Cap raised 6→40 over time (`breakout-discovery.ts:62`); `docs/audit/INTENTIONAL-DESIGN.md:99` and `docs/audit/MONDAY-RTH-READINESS.md:48` are STALE (still say 15 and 6 respectively) — doc-drift flagged separately.
2. **FLOW** (`scan.ts:247`, `flow-ingest.ts:84`) is technically whole-market (no ticker filter) but gated by a **$200k single-print premium floor at ingestion** (`UW_FLOW_MIN_PREMIUM ?? 200_000`) — too high for 0DTE contracts specifically, whose absolute premium-per-contract is much smaller than swing/LEAPS options even for a genuinely large directional bet.
3. **PIN** (`pin-discovery.ts:6-11,51-59`) is explicitly NOT whole-market by design — a hardcoded 30-ticker allowlist (indices + mega-caps only), because it's a mean-reversion fade requiring `gammaPosture === "long"` (range-bound tape) — the opposite regime of a trending small-cap breakout. Confirmed this is intentional, not a bug; expanding its universe would not surface bangers (they'd fail the regime test regardless) and risks feeding it unreliable per-strike GEX data from illiquid names.
4. **Contract selection** (`deterministic-edition.ts:213,286,301-302`) is deliberately ATM/near-the-money (safety over torque) — caps %-return-per-$-of-underlying-move independent of everything else.
5. **Exit engine has a hard +100% ceiling** on the 0DTE directional ledger (`plan.ts:18` `target_pct:100`, `exit-engine.ts:378-386` auto-banks the runner in full the instant it tags +100%; outcome enum literally only goes up to `"doubled"`, `exit-engine.ts:317`) — a play cannot record >+100% even if the underlying keeps running. A SEPARATE engine (`scale-out.ts`, "Engine B" in `0DTE-RESEARCH.md`) has no such ceiling and is where real historical 5-23x winners came from (ANET $0.36→23.3x, `0DTE-RESEARCH.md:150`) — but it is NOT wired into the live 0DTE board today, only partially cron-wired into the nighthawk ledger.
6. **Gate funnel bias**: G-8 (anti-chase, `CHASE_PCT=55%`) and G-9 (illiquidity, spread>15%) are the two gates most likely to reject a genuine small-cap outlier (already-run-up premium, thin/wide options market) vs a calm mega-cap. Working hypotheses about G-12 confluence and the governor's concurrent-plan cap being biased were WRONG on inspection — confluence is actually *easier* for single names (1 confirmation needed vs 2 for index/ETF) and the concurrent cap defaults to 100 (explicitly "no scarcity" by design).
7. **AMIX** (+235% today, the session's biggest mover) had **zero listed 0DTE option contracts** — benign, structural, unrelated to any gate/bias; some names simply have no same-day expiry.

**Live re-measurement (2026-08-04, today's session, current code):** `discovery-recall-probe.mjs --grade=2026-08-04 --scan-top=100` → `total_qualifying_movers: 40` exactly equals `keep_cap: 40` → **0 dropped, 0 recall miss.** The cap was NOT binding today — raising it further would have made zero difference this session. The 2026-07-25 "leaky top-6" finding is STALE (measured the old cap/rank); today's cap=40/momentum-rank config shows no leakage, at least for this one session. Recommend re-running across several more sessions (varying breadth) before concluding the cap is never the bottleneck — one session is not a trend.

**Status:** Research/diagnosis only — no code changed. Two concrete, agreed next steps pending: (a) lower/rework the FLOW premium floor for 0-1DTE specifically (single-print $200k floor too high for 0DTE premium economics — considering an accumulated-premium basis instead of single-print), (b) do NOT expand PIN's universe (confirmed correct as designed); the real lever for "no big winners" is scoping/finishing Engine B's (whole-market banger scale-out) wiring into a visible board, a larger separate initiative not undertaken today.
