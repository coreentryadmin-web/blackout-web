...
## 2026-08-04 — [P2, 0DTE, FLOW discovery] Flat $200k premium floor structurally biased FLOW toward SPY/QQQ/mega-caps — 0DTE-specific floor added

**Trigger.** Operator, following the multi-agent whole-market architecture research (see the companion "why does whole-market 0DTE scanning never produce a 100%+ winner" entry): "I agree, 100k single print on 0dte is very rare... how do we fix this up?" — agreed to build the fix live.

**Root cause.** `flow-persist.ts`'s `MIN_PREMIUM = Number(process.env.UW_FLOW_MIN_PREMIUM ?? 200_000)` was the single canonical premium floor applied UNIFORMLY to every flow print regardless of DTE, at every ingestion path (`flow-ingest.ts`'s REST cron, `uw-socket.ts`'s `flow_alerts`/`option_trades` WS channels, and the reconciliation backfill), with `persistAndPublishFlowAlert`'s `if (flow.premium < MIN_PREMIUM)` as the final authoritative gate. A $200k floor is reasonable for a swing/LEAPS contract, but a 0DTE contract's absolute premium-per-contract is far smaller even for an equally large DIRECTIONAL conviction bet — most of the extrinsic value has already decayed by mid-session. A genuinely large 0DTE bet on a mid-cap name could easily total under $200k in one print and never enter the pipeline `scan.ts`'s FLOW discovery reads from at all, while SPY/QQQ/mega-cap prints (naturally larger dollar notional per contract) always cleared it — structurally biasing FLOW toward the same handful of liquid names every session, independent of any gate/scoring logic downstream.

**Evidence.** Live-verified via the earlier architecture research: FLOW discovery's SQL query (`scan.ts:247`, `min_premium: 100_000, max_dte: 1`) — itself downstream of, and moot relative to, the $200k ingestion floor — confirmed as the mechanism. `flow-persist.ts:18`, `flow-ingest.ts:84`, `uw-socket.ts` lines ~1233/1315/1480 all traced to the same single `MIN_PREMIUM` constant.

**Fix.** Added a DTE-aware floor in `flow-persist.ts` (the one place every ingestion path already funnels through): `MIN_PREMIUM_NEAR_DATED` (env `UW_FLOW_MIN_PREMIUM_0DTE`, default $50,000 — a v1 constant, tune with calibration data like this codebase's other thresholds), a `requiredMinPremium(dte)` helper (near-dated floor for dte≤1, standard `MIN_PREMIUM` otherwise, conservatively defaults to the HIGHER floor when dte is null/unconfirmed — never guesses near-dated), and `dteFromExpiry(expiryYmd)` (derives DTE locally from `MarketFlowAlert.expiry`, since the type carries no precomputed `dte` field; kept local to avoid coupling this persistence module to `horizon-fanout.ts`'s `calendarDte` for one date-diff). `persistAndPublishFlowAlert`'s gate now uses `requiredMinPremium(dteFromExpiry(flow.expiry))`. Every UPSTREAM fetch/subscribe call (`flow-ingest.ts`'s REST cron, `uw-socket.ts`'s reconciliation fetch, `flow_alerts` WS pre-filter, `option_trades` WS pre-filter) now fetches/pre-filters at the BROADER `MIN_PREMIUM_FETCH_FLOOR = min(MIN_PREMIUM, MIN_PREMIUM_NEAR_DATED)` so near-dated prints between the two floors actually reach the precise persist-time check instead of being discarded upstream before persist ever sees them. `scan.ts`'s own SQL query floor (previously a hardcoded `100_000`, which would have re-excluded the newly-admitted $50-100k rows) now imports and uses `MIN_PREMIUM_NEAR_DATED` directly.

**Blast radius.** `option_trades` WS channel's initial pre-filter (a raw-trade-tape ring buffer, `optionTradesStore`, feeding non-gating display consumers) is also broadened to the lower fetch floor as an accepted side effect — the DTE-aware precise check still gates persist immediately afterward; this channel also feeds `flow_alerts` via the same persist call so it's in-scope, not incidental. `dedupeByKindLevel`/gate/scoring logic downstream of FLOW discovery is untouched — this fix only widens what data FLOW discovery can see, it does not change any gate threshold, scoring formula, or commit logic.

**Tests.** 12 new cases in `flow-persist.test.ts`: `MIN_PREMIUM_NEAR_DATED` strictly lower than `MIN_PREMIUM`; `requiredMinPremium` correctly branches on 0/1 DTE vs 2+ DTE vs null/NaN (conservative default); `dteFromExpiry` for same-day/next-day/a-week-out/unparseable; two end-to-end cases proving a $75k same-day print now clears the floor (would NOT have under the old flat $200k) while a $75k 30-day print still correctly needs the full standard floor (fix doesn't weaken longer-dated ingestion). All 16 tests in `flow-persist.test.ts` pass; all 22 in `zerodte/scan.test.ts` pass (`--experimental-test-module-mocks`). `npx tsc --noEmit` clean (pre-existing unrelated `.next` build-cache errors excluded).

**Status:** FIXED — PR opened at the operator's explicit request (not the standing PR-only rule — built and tested live), not merged.
