...
> **Full system guide (architecture → real-time operation):** [`SWING-SYSTEM.md`](./SWING-SYSTEM.md)
...
**GO-LIVE — 2026-07-24 (branch `feat/swing-commit-roll-live`, REAL MONEY, HELD for lead review, no auto-merge).** The operator authorized the lane live ("GO — everything live, not paper-first"). The three deliberate holds are lifted behind FOUR conjunctive hard rails: a WATCH candidate opens a REAL position IFF (1) its archetype×sub-lane bucket has GRADUATED through the SHIPPED staged Wilson-LB ladder (both floors — `analyzeArchetypeRecord` ∧ `analyzeSubLaneRecord`, zero new math), (2) it clears the ARMED portfolio budget (`PRODUCTION_PORTFOLIO_BUDGET`: $100k reference account, 2% per-trade / 6% total heat / 3% event / 4% overnight, `enforce:true`), (3) it clears the book-percent caps (`allocateSwingBook`), and (4) idempotency (`commit_key`). `commitEligibleCount` is now the REAL graduated count (was a literal 0). The roll executor is wired into the active-refresh cron (`roll-plan.ts` → grade parent from the live mark + gated further-out child). Cold book ⇒ nothing graduates ⇒ nothing commits — the rail, not a bug. New modules: `src/lib/swing/commit.ts`, `src/lib/swing/roll-plan.ts`; details in `docs/audit/FINDINGS.md` (2026-07-24 GO-LIVE).
...
- **`swing_candidate_accumulation`** — PK `(ticker, direction, archetype)`; persistence memory (observation count, distinct session days, phases_seen, signal_kinds, promoted_position_id). `UNCLASSIFIED` holds live-flow / pre-classify sightings. Accessors: `upsertSwingAccum`, `fetchAccumulating`, `markAccumPromoted`, `fadeStaleAccum`.
