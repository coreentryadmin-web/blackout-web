...
### 61. Ask Largo swing brief — 7 coaching helpers + wall-dynamics section + trade-manager proximity/wall-event bullets still narrated from stale Vector snapshots — #4413 — 2026-09-06

- **What was broken:** #4400–#4402 gated `chartTechnicalsSection`, `chartLevelsSection`, `watchForSection`, `vectorDeskSection`, `vectorPlayCoaching`, `collectFocalLevels`'s king strike, and `counterThesisLine`'s walls against `vectorSnapshotStale()`. Seven coaching helpers in `play-brief-narrative-coaching.ts` were never covered: `magnetCoaching`, `expectedMoveCoaching`, `confluenceCoaching`, `wallIntegrityCoaching`, `vexCoaching`, `flowPrintsCoaching`, `wallDynamicsCoaching` — all still narrated directional/level content straight off `vec.*` fields with no staleness check. `wallDynamicsSection` (the intel-section counterpart in `play-brief-intel.ts`) had the identical gap, and `tradeManagerNarrativeSection`'s own "Nearest wall"/"Wall just moved" bullets (built from `vec.proximity`/`vec.wallEvents` directly) were likewise ungated. Four independent PRs (#4404-#4407) converged on parts of this gap in parallel; #4407 additionally reverted #4402's already-shipped fixes, so the final fix was manually reconciled on top of `main` (which already has #4402) rather than merging #4407 as-is.
- **What changed:** Added `if (vectorSnapshotStale(vec, Date.now())) return null;` (or the equivalent gate) to all 7 coaching helpers and to `wallDynamicsSection`; gated `tradeManagerNarrativeSection`'s proximity/wall-event bullets behind the same check. Narrative/coaching text only — no API/schema changes.
- **RTH check:** Open Ask Largo on a swing row with a stale Vector snapshot (>120s / `freshness: "stale"`) carrying a gamma magnet, expected-move band, confluence zone, wall-integrity tier, VEX flip, flow print, or wall-dynamics event — confirm none of those coaching bullets render. Also confirm the Trade manager read's "Nearest wall"/"Wall just moved" bullets are absent on the same stale-snapshot row.
