...
- **RTH check:** PR #5493 merged 2026-09-25 08:33 UTC; ECS deploy not yet confirmed as of this entry (production still serving task def `:1791`, predating the merge). Once deployed, pull a live play-brief for any of ALAB/CRDO/KLAC while 2+ semis/AI-infra names (including LRCX/INTC/NVDA/AVGO/MRVL) are concurrently open/committed same-direction, and confirm the Book-context section now names the shared `semis` concentration instead of omitting it.
