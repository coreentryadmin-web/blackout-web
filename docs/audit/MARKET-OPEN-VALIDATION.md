...
### 310. Night Hawk Swings: book-context concentration warning rendered a duplicated non-reviewed ticker identically twice — fix/swing-book-context-dup-ticker — 2026-09-20

- **What was broken:** `bookContextSection`'s concentration line (`play-brief-intel.ts`) already had disambiguation logic for "two genuinely separate positions on one ticker" (2026-09-15/09-18 fixes), but it only ever fired for the REVIEWED play's own ticker duplicating in its own overlap list. Live-confirmed on RIOT (crypto-equity theme, 11-position concentration): a real swing-native MSTX position and a real, independently-committed cross-engine (Banger) MSTX sibling both genuinely overlap RIOT's theme, and both rendered as textually identical "MSTX LONG" with no distinguishing detail — reads exactly like a duplicate-counting defect even though the underlying count is honest.
- **What changed:** `formatOverlapPosition` now also disambiguates any ticker that appears more than once in the SAME rendered list (via a new `tickersAppearingMoreThanOnce()` helper), additive to the existing "this is the reviewed ticker" rule which still fires unconditionally (a single cross-engine sibling on the reviewed ticker itself still reads as self-citation-like at count 1, unrelated to duplicate counting).
- **RTH check:** once deployed, pull a live play-brief for a ticker whose book-context concentration line includes a theme with 2+ real cross-engine siblings on some OTHER ticker in the book (crypto-equity is the densest theme currently — RIOT, MARA, MSTX, MSTR, COIN, CLSK, HUT, IREN, GLXY, SBET, MSTU) and confirm any duplicated ticker in that line now carries a distinguishing `(separate position #N)` / `(separate, cross-engine position #N)` suffix rather than appearing as a bare, identical repeat.

### 311. Ask Largo swing play-brief: archetype/ticker track-record numbers also surface as a citable evidence[] entry, not just prose — enhancement, not a bug — fix/swing-track-record-evidence — 2026-09-20

- **What was missing:** `archetypeTrackRecordSection`/`tickerTrackRecordSection` (`play-brief-intel.ts`) already cite Largo C10 ("historical context") in their own doc comments and render real wins/losses/n numbers — but only into the "Track record"/"Ticker track record" SECTIONS' prose, never as a structured `evidence[]` entry with its own provenance the way every other citable fact in `evidenceFromContext` already is. Found via a deliberate, systematic sweep of `LARGO-PRODUCT-CONTRACT.md`'s C1-C10 points this cycle, not a live defect repro.
- **What changed:** two new additive `evidenceFromContext()` (`play-brief.ts`) pushes carrying the SAME numbers the prose sections already show, with real provenance (`source: "Swing ledger"`) — archetype track record gated on `graduatedArchetypeEntry` (identical gate to the prose section, so an ungraduated bucket is still correctly never cited), ticker track record gated on `priorClosedTrades > 0`. The two prose sections are completely untouched.
- **RTH check:** pull a live play-brief for a ticker with a graduated archetype bucket and/or prior closed trades on that specific ticker and confirm `envelope.evidence[]` now includes lines starting `"Archetype track record:"` / `"Ticker track record:"` alongside the existing prose sections, with matching win/loss numbers and `provenance.source: "Swing ledger"`.

### 312. Book concentration (`checkPortfolioOverlap`) undercounting real crypto-correlated exposure — crypto-equity theme map missing exchanges, miners, and BTC/ETH trust/ETF wrappers — fix/crypto-equity-theme-coverage — 2026-09-20

- **What was broken:** `sector-map.ts`'s curated `crypto-equity` theme list had never been extended to cover a whole class of crypto-correlated tickers live on the board right now — crypto exchanges/custodians (GEMI, BKKT, ABTC, BLSH), bitcoin miners (BMNR, BTDR), and direct BTC/ETH trust/ETF wrappers (ETH, ETHE, ETHU, ETHA, IBIT, GBTC, BITO, BITX). Live-confirmed: a real 2026-09-18 Banger-promoted commit batch of 36 positions, most of them crypto-correlated, had its own "Book overlap" evidence line reporting only 11 same-direction crypto-equity positions — a member reading that number saw a fraction of the real concentration risk actually in the book.
- **What changed:** added the 14 tickers above to `SECTORS["crypto-equity"]` (`sector-map.ts`), following the exact established pattern from the prior MSTU/MSTX/GLXY/SBET addition — purely additive data, no logic touched in `resolveTheme`/`sameThesis`/`checkPortfolioOverlap`. Deliberately left out ambiguous partial-correlation names (HOOD, CRCL, APLD, BULL) rather than over-reaching.
- **RTH check:** pull a live play-brief for RIOT (or any other crypto-equity-themed ticker) and confirm the "Book overlap" evidence line's same-direction crypto-equity count has risen from 11 to reflect the newly-included tickers actually still live in the book at that time (count will vary as positions close/roll — confirm the NEW tickers, e.g. GEMI/BKKT/ETH/IBIT, are now counted, not a specific fixed number).

### 313. UW multiplex socket reconnect-storm off-hours — first-message grace never widened for off-hours like the sibling stall window already is — fix/uw-socket-first-msg-grace-offhours — 2026-09-20

- **What was broken:** live CloudWatch Logs (`/ecs/blackout-production-market-worker`) showed `[uw-socket] stall watchdog — OPEN 44s with ZERO messages, reconnecting (possible API-key contention)` firing on a tight ~44-45s cycle continuously across a 6+ hour sampled window on a Sunday (fully off-hours — no RTH, no session). `runUwReconcileTick()` already widens the stall window for off-hours (`UW_SOCKET_STALL_MS` 75s → `UW_SOCKET_STALL_OFFHOURS_MS` 5min) for the "has delivered a message before" branch, but always passed the RTH-tuned 30s `UW_SOCKET_FIRST_MSG_GRACE_MS` unconditionally for the "never delivered ANY message yet" branch — so a socket that legitimately sees zero off-hours traffic tears itself down and reconnects every ~30-45s forever, hundreds of times over a weekend. The log's own "(possible API-key contention)" guess is backwards: the churn itself is the more likely source of any real contention.
- **What changed:** added `UW_SOCKET_FIRST_MSG_GRACE_OFFHOURS_MS = 5 * 60_000` (`uw-socket-stall.ts`) and select it in `runUwReconcileTick()` the same way `stallMs` already selects between RTH/off-hours — mirrors an existing pattern rather than introducing a new one. RTH behavior (30s grace, the genuine duplicate-API-key-silent-accept detector) is completely unchanged.
- **RTH check:** this is primarily an OFF-hours fix, so the most direct check is off-hours: during the next evening or weekend window after this deploys, grep `/ecs/blackout-production-market-worker` CloudWatch Logs for `stall watchdog` and confirm the tight ~45s reconnect loop is gone (a socket should now stay open ~5 minutes before any off-hours-silence reconnect, if it reconnects at all). Separately, during the next RTH session, confirm the stall watchdog still fires promptly (within ~30-75s) on any genuine stall — this fix must not have blunted real-time fault detection during live trading hours.

### 314. Ask Largo swing play-brief: "Book context" concentration line dumps every overlapping ticker with no cap — fix/swing-book-context-name-cap — 2026-09-20

- **What was broken:** `bookContextSection` (`play-brief-intel.ts`) joins every same-theme overlap name into one prose sentence with no length limit. Live-confirmed on BKKT (crypto-equity theme, 28-position concentration): the "Book context" section rendered one unbroken 28-ticker comma-separated wall — a raw data dump, not trader-manager prose, and the exact "bullet-dump instead of narrative" failure mode the Ask Largo standing mandate calls out. Every prior fix to this function (2026-09-06 → 2026-09-18) disambiguated individual name collisions but never bounded list length.
- **What changed:** added `MAX_OVERLAP_NAMES = 8` and a `joinOverlapNames()` helper — renders the first 8 formatted names, folds the rest into `+ N more`. The lead sentence's own true count stays honest (unchanged); only the exhaustive namewall trims. Disambiguation still runs over the full list before truncation.
- **RTH check:** pull a live play-brief for any ticker in a dense same-theme book (crypto-equity is currently the densest — RIOT/BKKT/MARA/COIN/etc.) and confirm the "Book context" concentration line now shows at most 8 names followed by `+ N more`, with the lead count unchanged from the true overlap total.

### 315. Ask Largo swing play-brief: "next trim" distance bullet used the entry-fallback mark as if it were a real live quote — fix/manage-lifecycle-coaching-fake-mark-basis — 2026-09-20

- **What was broken:** `manageLifecycleCoaching`'s "next trim" distance-to-target clause (`play-brief-narrative-coaching.ts`) computed its "mark $X, needs $Y (+Z% from here)" figure from `play.mark`/`play.execMark` with no check for whether `play.mark` was a real quote or the pure entry-premium fallback banger-lane positions carry before their first live sync. Live-confirmed on WOLF (`SWING:WOLF:1218`, OPEN, 2026-09-20): "Trade manager read" stated "next trim at +100% — mark $0.77, needs $1.54 (+100% from here)" while the SAME envelope's "Position" section, a few lines above, correctly read "Mark: unknown (sync quote, no live price yet — do not read as flat)" for the identical $0.77 value — a specific room% computed from the exact number the document says is not known.
- **What changed:** the distance clause now gates on the shared `optionMarkGenuinelyUnknown` helper (the same one `pnlSection`'s "Mark" line and two `play-brief-intel.ts` rail lines already use) — when the mark is the true entry-fallback echo, only "next trim at +100%" renders, with no fabricated dollar-distance clause. The 2026-09-18 BASIS MISMATCH fix (prefer execMark over mid when both are real) is unchanged.
- **RTH check:** pull a live play-brief for a banger-lane OPEN position that has never synced a live option quote (Position section reads "Mark: unknown") and confirm its "Trade manager read" no longer states a specific "mark $X, needs $Y" distance for the next trim rung — just the bare "next trim at +N%". Separately, confirm a position WITH a real synced mark (Position section shows a real dollar Mark) still shows the full distance clause exactly as before.

### 316. Night Hawk Swings: closed positions have no real multi-truth grade anywhere — new admin retrace endpoint closes the "retrace vs the 5-truth grader" audit gap — fix/swing-multi-truth-grade-retrace — 2026-09-21

- **What was broken/missing:** `swing/grade.ts`'s real multi-truth grader (`gradeSwingPosition` — EXECUTION/PATH/THESIS/MANAGEMENT/FINANCIAL) has zero non-test production call sites (re-verified against current `main`). Every closed swing position members see is graded only by `gradeParentFromMark`'s single point-in-time markfreeze P&L freeze — a same position can show a small positive realized P&L while its underlying THESIS actually broke (saved by a stop/roll), or a large unrealized MFE the exit never captured, and none of that was visible anywhere. `OUTCOME-GRADING-SPEC.md` §6 documented this gap (PR #5023) but it was never acted on; re-surfaced on #4076 (comment 5751117208).
- **What changed:** new admin-only, read-only GET route `/api/admin/swing/multi-truth-grade` (`days` param) that fetches closed/rolled `swing_positions` rows, fetches real Polygon forward underlying daily bars per position's committed→closed window, and calls the REAL `gradeSwingPosition` against them — returning PATH + THESIS truths (EXECUTION/FINANCIAL/MANAGEMENT stay honestly `ungradeable` this pass, no real fill price or option bars fetched) alongside each row's frozen markfreeze P&L for direct comparison. Pure row-mapping logic lives in `src/lib/swing/grade-retrace.ts` (unit tested). Companion audit script: `scripts/audit/swing-multi-truth-grade-retrace.mjs` — reports gradeable rates, THESIS outcome distribution, and flags rows where THESIS disagrees with the sign of the markfreeze P&L. No schema change, no cron/infra dependency, no existing call site touched.
- **RTH check:** once deployed, run `node --import tsx scripts/audit/swing-multi-truth-grade-retrace.mjs --days=90` against production and confirm the route returns real graded rows (not 404/502), PATH/THESIS gradeable rates are nonzero for the closed population, and review any reported THESIS-vs-markfreeze divergences for genuine signal (e.g. a position whose thesis was actually INVALIDATED but which still closed with non-negative P&L via a well-timed stop/roll).

### 317. `GET /api/market/nighthawk/horizons?view=swings` destroyed real gamma/theta/vega/iv to 0.00 at the response-boundary rounding — fix/horizons-greek-precision — 2026-09-21

- **What was broken:** the route's `roundFloats(payload, 2, keyDp)` call fixed `mid`/`entryPremium`/`peakPremium` precision but the `keyDp` map never actually included `gamma`/`theta`/`vega`/`iv` — despite the route's own comment citing `round-floats.ts`'s documented gamma-quantization hazard as precedent. Live-confirmed on the flagged AAPL swing position (`positionId 37`, 330C, 1DTE): `GET /api/market/nighthawk/horizons?view=swings` served `gamma:0, vega:0, iv:0` exactly, while a live Polygon re-query of the same contract returned meaningfully-sized greeks (`gamma:0.0417, vega:0.0562, iv:0.4848`) for it. The greeks were carried honestly through the whole pipeline (`live-plays.ts` never fabricates — absent quote reads `null`, not `0`); this was purely a lossy-rounding defect at the response edge, real data reading as a fabricated confident zero — a Largo product-contract precision violation.
- **What changed:** added `gamma`, `theta`, `vega`, `iv` to the route's `keyDp` map at 4dp (matching the documented precedent and the identical fix already shipped for Vector, `vector-response-rounding.ts`). `delta` left at 2dp — already meaningful at that precision.
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` for any committed swing position with a live quote and confirm gamma/theta/vega/iv render at real precision (not a flat `0` for a position that plausibly has a nonzero small greek). Separately worth a follow-up look (not fixed this cycle, disclosed in the staged finding): the flagged AAPL position's `markAsOf` was 3 days stale on its own expiry day — check whether `swing-active-refresh` is genuinely re-quoting every OPEN position every RTH tick.

### 318. BANGER-origin swing positions never tracked `trough_premium` — Ask Largo Position card always showed "Trough: —" for the majority of the live book — fix/banger-trough-premium-untracked — 2026-09-21 (entry added retroactively; PR #5333 merged without one)

- **What was broken:** `banger_positions` only ever had a `peak_premium` column — no `trough_premium` anywhere in the schema, latch, or the three consumers that read it (`BangerPositionRow`/`mapBangerPositionRow`, `horizonPlayFromBangerPosition`, `bangerRowToActivePlay` — the last of which hardcoded `trough_premium: null` outright). Live-confirmed on RBRK (`positionId 1128`, entry $2.05, mark $2.00, genuinely below entry): the play-brief correctly rendered `Peak: +91.5%` but `Trough: —` despite the live P&L (-2.4%) proving the mark had been underwater. Also silently degraded the closed-play "Drawdown before outcome" narrative for every BANGER-origin closed position.
- **What changed:** added the `trough_premium` column (mirrors `swing_positions`' existing `GREATEST`/`LEAST` peak/trough latch pattern exactly), wired it through all three consumers. **Rows written before this migration keep `trough_premium` NULL forever** (an `ALTER ADD COLUMN` cannot backfill history that was never recorded) — a row only starts filling in on its next real live-marks tick.
- **RTH check — this is a genuinely delayed validation, not an immediate one:** re-checked RBRK live post-deploy (2026-09-21, still off-hours/weekend, `markAsOf` still stuck at 2026-09-18 16:00 ET — no fresh tick has landed since the fix shipped) and confirmed `Trough: —` is *still* showing, exactly as expected per the fix's own "NULL forever until the next tick" design — **this is NOT evidence the fix failed**, it is evidence no live tick has happened yet. Once RTH resumes and `swing-active-refresh` gets a fresh option quote for a BANGER-origin position that ever prints below entry, confirm its Position card starts showing a real `Trough: X%` instead of `—`. If a position with an obviously-adverse P&L still shows `Trough: —` well into a live RTH session (several ticks in, not the first one), that would be the real signal to re-open this.

### 319. Ask Largo swing play-brief: "Thesis health" section permanently withheld for every committed position — the entry-geometry pillar was never live-derived (only its sibling, persistence, was) — fix/swing-thesis-health-entry-geometry-wire — 2026-09-21

- **What was broken:** `thesisHealthUncalibrated()` (`thesis-health.ts`) withholds the WHOLE aggregate thesis-health score if ANY one of five pillars is still on its generic default label (an OR across pillars). A 2026-09-20 fix already re-derives the persistence pillar live for committed positions (`liveSetupState` in `adapters.ts`, since `src.setupState` never survives the WATCH→COMMIT transition) — but the sibling entry-geometry pillar (`src.entryStatus`) has the identical structural gap and was never fixed: `live-plays.ts`'s committed-row `HorizonPlay` literal never sets `entryStatus` at all, so it always defaults to `"n/a"`, which by itself kept tripping the whole withheld-aggregate path. Live-confirmed 2026-09-21: HOOD, SNOW, SMCI, and CRWD play-briefs all still showed "Inputs not wired for committed positions" despite setupState/signalKinds both being wired.
- **What changed:** exported `deriveEntryState` (`entry-model.ts`, previously module-private) and added `liveEntryStatus` in `adapters.ts`, computed the same way `liveSetupState` already is — reusing the identical `liveSpot`/`entryTriggerUnderlyingPx` legs, falling back to the (still-absent) `src.entryStatus` when they're missing. Wired into `computeSwingThesisHealth`'s `entryStatus` field.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief` for any OPEN/HOLD/TRIM swing position with a real `liveSpot`/entry-trigger price on record and confirm the "Thesis health" section now renders a real `**N%** · <rung label>` line with a pillar breakdown, instead of "Inputs not wired for committed positions — aggregate score withheld". Also confirm the "Entry geometry" pillar's label is one of `at trigger`/`pullback`/`pre-trigger`/`chase risk` (not `n/a`) on those rows.

### 320. Ask Largo swing play-brief: "Ticker track record" cited a chain that closed AFTER the reviewed play as "prior" evidence — fix/swing-ticker-track-record-temporal-order — 2026-09-21

- **What was broken:** `loadTickerTrackRecord()` (`play-brief-ticker-history.ts`) renders "the desk has traded TICKER N time(s) before this play: WW/LL" but only self-excluded the reviewed play's OWN chain — it never checked that a candidate chain actually resolved BEFORE the reviewed play. Live-confirmed on two real closed EWZ chains: `SWING:EWZ:26` (STOPPED, closed 2026-08-28 — chronologically the FIRST EWZ trade) rendered "1W / 0L", citing `SWING:EWZ:29` (a WIN closed 2026-09-03, i.e. LATER) as if it had already happened; #29's own brief correctly cited #26 as "0W / 1L". The two briefs directly contradicted each other about the same two trades.
- **What changed:** `loadTickerTrackRecord` now takes an optional `asOfMs` (the reviewed play's own resolution instant, from `TerminalPlay.exitAt`; `null` for a still-open play — no cutoff needed). Each candidate chain's own resolution time (latest `closed_at ?? graded_at` across its legs) is compared against `asOfMs`; a chain resolving at or after it is dropped. An undated candidate is kept, not dropped (best-effort, never fabricate absence). `record.ts`'s all-time aggregate helpers (`/record`, `closedDeckSourcesFromChains`) are unchanged — they correctly have no "as of" concept.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief` for the two chronologically-adjacent closed positions on the SAME ticker that were used as this fix's repro (or any similar real pair once one exists in the live closed population going forward) and confirm the earlier-closing one's brief no longer cites the later one's outcome as "prior" — i.e. it should show no "Ticker track record" section at all if it was genuinely the first trade on that ticker, while the later one's brief continues correctly citing the earlier one.

### 321. Night Hawk Legacy: contract selection defaulted to full ATM regardless of underlying price — an expensive stock's pick could cost 10-50x a cheap stock's for no reason related to setup quality — fix/legacy-contract-cost-preference — 2026-09-21

- **What was broken:** `pickChainContract` (`deterministic-edition.ts`) always picked the strike nearest to spot (ATM), checking only that premium cleared the hard `MAX_OPTION_PREMIUM_PER_SHARE = $35/share` ceiling — a ceiling that exists (PR-N15) so a strong candidate is never dropped outright, never an affordability target. Since ATM premium scales roughly linearly with the underlying's own price, the identical "closest to spot" rule produced wildly different dollar costs across tickers in the same book. A member flagged this directly, pointing at the real published book: `MU $1020 CALL @ $24.88` ($2,488/contract) next to `BMNR $26C @ $1.11`, `ETHA $20C @ $0.52`, `MARA $13C @ $0.71` (all $52-$111/contract).
- **What changed:** added a strictly opt-in preference tier — `pickChainContract` gained a 5th optional parameter `preferAffordable`. When true, it first tries the subset of the already-strict (liquid + under the hard cap) pool that also clears `premium ≤ PREFERRED_OPTION_PREMIUM_PER_SHARE ($8)` AND `|delta| ≥ MIN_PREFERRED_CONTRACT_DELTA (0.15)`, picking the nearest-to-spot strike within that subset; falls straight through to the exact unchanged ladder when nothing clears both (including when the row has no delta at all). All 4 Legacy call sites opted in; Vector's calls to the same shared function are untouched (new trailing optional param only). Both constants were grounded in a live MU chain pull (spot $1017.75): 0.15Δ ≈ $550-650/contract (still a real directional trade), while the $100-150/contract range other tickers sit at natively only appears below ~0.06Δ on MU (genuine lottery territory).
- **RTH check:** once a future edition's discovery pool actually contains a genuinely expensive underlying (roughly >$500/share) as a candidate, pull that night's `GET /api/market/nighthawk/edition` and confirm its published contract lands meaningfully cheaper than a plain ATM pick would (or, if nothing on the chain clears both the $8/share and 0.15Δ bars, that it honestly falls back to the ATM pick rather than silently picking something worse). Tonight's own book (MU/BMNR/ETHA/MARA) was already committed before this fix shipped, so it won't show the effect — this needs the *next* new edition build with an expensive-underlying candidate to observe live.

### 322. Ask Largo swing play-brief: "Confluence" and "Structural support/resistance node" could disagree on the SAME put/call wall within one brief — independently-sampled staleness clocks — fix/swing-play-brief-staleness-anchor — 2026-09-21

- **What was broken:** `composeSwingPlayBrief` samples `Date.now()` once at the top but never threaded it anywhere — every downstream staleness check (`vectorSnapshotStale(vec, Date.now(), ...)`/`gexMatrixStale(..., Date.now())`, ~20 call sites across `play-brief.ts`/`play-brief-intel.ts`/`play-brief-narrative.ts`/`play-brief-narrative-coaching.ts`) independently samples the real wall clock at whatever instant that particular section happens to compose — real seconds apart within one request once Meridian/book-context/track-record reads run in between. Live-confirmed on SNXX (`SWING:SNXX:1196`, 2026-09-21): "Trade manager read"'s `confluenceCoaching` bullet said "Confluence 15.50 (put-wall@15.5 ...)" while "What to watch"'s `watchForSection` said "Structural support node: put wall 10.00" — both reading the identical `vec.gexWalls.putWalls` on the identical, already-fetched Vector snapshot, disagreeing only because one section's staleness check landed just inside the 120s `VECTOR_STALE_MS` window and the other's landed just outside it.
- **What changed:** added `SwingPlayBriefContext.readMs?: number | null`, stamped once at the top of `composeSwingPlayBrief` before any section composes. Wired the two call sites this live repro exercised — `watchForSection` now uses `ctx.readMs ?? Date.now()`; `confluenceCoaching` gained an optional trailing `readMs?: number` param (defaults `Date.now()`, so every other call site/test is unaffected) and its one call site now passes `ctx.readMs ?? undefined`. The other ~18 `Date.now()` call sites are the identical latent-risk shape and are a documented follow-up sweep, not folded into this fix.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief` for a committed position during a live RTH session (when Vector data ages normally rather than sitting fully fresh or fully stale) and confirm "Trade manager read"'s Confluence bullet's underlying primary wall and "What to watch"'s Structural support/resistance node cite the SAME price when both cite the top-ranked put/call wall (disambiguated `kind@price` labels are still expected and correct when the confluence engine genuinely picked a lower-ranked wall — only an UNLABELED disagreement on the primary wall would indicate this fix didn't hold).

### 323. Ask Largo swing play-brief: "Data freshness" narrative and `unavailableSources[]` chips could disagree on the SAME mark/Vector/GEX staleness within one brief — #5351 follow-up, same independently-sampled-clock defect on two more sections — fix/play-brief-readms-followup — 2026-09-21

- **What was broken:** #5351 (finding #322 above) added `ctx.readMs` and wired it into the two sections its live SNXX repro caught disagreeing. It deliberately left ~18 other `Date.now()` staleness call sites as a documented follow-up. Two of the remaining sites were the highest-risk of that list because they sit directly opposite each other and opposite the already-fixed `watchForSection`: `dataFreshnessSection` (the narrative line "Option mark as of X" / "Option mark **stale**") and `collectBriefUnavailableSources` (the `unavailableSources[]` chips `UnavailableChip` renders for the SAME option-mark/Vector/GEX/Meridian/news fields) — neither had ever consulted `ctx.readMs` despite both already receiving `ctx`. A member could see a green "as of" narrative line next to a red "stale" chip for the identical `markAsOf`, or the reverse, purely from real compose-time clock drift.
- **What changed:** `dataFreshnessSection` now samples `const readMs = ctx.readMs ?? Date.now();` once and passes it to `optionMarkIsStale`/`vectorAgeStale`/`gexMatrixStale`/`gexMatrixAgeMs`. `collectBriefUnavailableSources` does the identical anchor and threads it through `collectOptionMarkStalenessAbsence`/`collectGexStalenessAbsence`/`collectVectorStalenessAbsence`/`meridianCatalystStale`/`newsCatalystStale`. All nine underlying helpers already accepted an optional `readMs` param — this was purely a wiring gap at the two call sites. The other ~16 remaining `Date.now()` sites (`wallDynamicsSection`, `vectorDeskSection`, and several coaching bullets in `play-brief-narrative-coaching.ts`) are the same latent shape and remain a further follow-up.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief` for a committed position with an option mark or Vector/GEX snapshot that is genuinely aging toward its staleness cutoff mid-request (harder to time deliberately — the natural opportunity is any live RTH position whose data happens to be near the 18-minute/120s boundary when checked) and confirm the "Data freshness" section's own staleness claims for mark/Vector/GEX agree with whether the matching `unavailableSources[]` chip (`UnavailableChip` in the UI) is present for the same field — no case of one saying fresh/present while the other says stale/absent for the identical field in the same envelope.

### 324. `bie-full-state-snapshot` cron had no cross-replica overlap guard — real runs exceeded its own 5-minute schedule (407.9s, 356.0s measured) with no protection against a doubled-up run competing for shared Polygon/UW rate limiters — fix/bie-full-state-snapshot-overlap-guard — 2026-09-21

- **What was broken:** standing performance/latency audit mandate sweep pulled live CloudWatch (`/ecs/blackout-production`, `elapsed=` grep, 2026-09-21 11:37-14:37 UTC RTH window) alongside ALB `TargetResponseTime` (p50 stayed under ~300ms all window, but p99 spiked to the tens of seconds — a tail-latency signature, not fleet capacity). The `elapsed=` grep surfaced `bie-full-state-snapshot` logging `elapsed=407874ms` and `elapsed=355974ms` against its own deployed 5-minute (`cron(*/5 11-21 ? * MON-FRI *)`) EventBridge schedule — both runs exceed 300s, meaning the next scheduled fire can start a second `buildBieFullState()` while the first is still in flight, contending for the same cluster-wide Polygon/UW rate limiters real member traffic depends on. The route had no overlap guard at all, unlike its sibling crons (`vector-pick-sweep`, `zerodte-warm`, `swing-discovery`, `banger-discovery`, etc.) which already use the `sharedCacheSetNx` pattern for this exact shape.
- **What changed:** added the identical `sharedCacheSetNx`/`sharedCacheDel` overlap-lock pattern `vector-pick-sweep` already uses — 900s TTL (matches the cron's existing `stale_after_min: 15`), fails open on a Redis error, releases in a `finally`. `vector-full-state-snapshot` (same 5-min schedule, max observed 197s) and `vector-dark-pool-warm` (10-min schedule, max observed 383s) were checked against the same window and are NOT at risk — correctly left untouched.
- **RTH check:** once deployed, re-grep CloudWatch `elapsed=` for `bie-full-state-snapshot` during a live RTH window and confirm no two runs' timestamps overlap (a run's start should always be after the prior run's own `elapsed=`-implied finish, or a `"previous snapshot still in flight"` skip line should appear in between). Also spot-check `GET /api/market/bie/full-state` (or wherever BIE reads `bie:full-state` from) still updates every ~5 minutes as before — the guard should only ever SKIP a genuinely-overlapping second run, never silently stop the cron from producing fresh data at all.

### 325. Ask Largo swing play-brief: `positionId`-based resolution silently ignored banger-origin positions — a specific requested leg always resolved to the highest-live-P&L leg on the same ticker — fix/swing-play-brief-banger-positionid-collision — 2026-09-21

- **What was broken:** `pickLanePlayForBrief` (the ticker-only lane fallback `play-brief-resolve.ts` falls into once its `swing_positions`-only lookups come up empty) never accepted a `positionId` hint at all — only `status`/`strike`/`right`. A banger-origin swing play's `positionId` is `banger_positions.id`, a namespace completely separate from `swing_positions.id` (per `banger-lane-merge.ts`'s own doc comment), so the earlier `swing_positions`-backed lookups can never match it, and every banger-origin `positionId` request fell into this blind fallback. With no strike/right hint (Largo's own `?playId=SWING:<ticker>:<positionId>` convention never supplies one), the fallback picked among all of the ticker's live legs by highest `livePnlPct` — the SAME answer regardless of which specific `positionId` was requested. Live-confirmed 2026-09-21 on ABTC's two real concurrent positions (1185: 9.5C TRIM +200%, 1220: 10.5C OPEN +25%): `?playId=SWING:ABTC:1220`, `?playId=SWING:ABTC:1185`, and `&positionId=1220` all returned the IDENTICAL 9.5C/1185 brief; only `&strike=10.5&right=C` resolved the correct 10.5C/1220 leg.
- **What changed:** `pickLanePlayForBrief` now accepts an optional `positionId` hint and checks for an exact match on it FIRST, before any strike/right/status/P&L heuristic — every `HorizonPlay` it sees already carries its own correct `positionId` (banger's real row id, or a real `swing_positions.id`), so an exact match is always trustworthy. Threaded through from `resolveSwingPlayForBrief`'s one call site.
- **RTH check:** once deployed, find a live ticker with 2+ concurrent banger-origin positions (`GET /api/market/nighthawk/horizons?view=swings`, look for `signalKinds: ["BANGER"]` rows sharing a ticker) and pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>:<id>` for each `positionId` separately — confirm each returns ITS OWN entry/mark/P&L/management data, not a shared answer that only changes when a strike/right hint is also supplied.

### 326. Ask Largo swing play-brief: lane-rank comparison treated a fabricated `score: 0` fallback as a real, comparable value — a real +39.2% HOLD winner rendered as the lane's worst-scored play — fix/swing-lane-rank-score-withheld — 2026-09-21

- **What was broken:** `livePlayFromSwingPosition` (`live-plays.ts`) falls back to a literal `score: 0` when a committed position's `feature_vector.evidence_score` was never pinned (an older commit predating the pin). `computeLaneRank` (`play-brief-lane-rank.ts`) sorts ALL peer rows by this raw `score` with no awareness the `0` can be a fallback rather than a measurement. Live-confirmed on `SWING:AAPL:40` (2026-09-21, RTH): a real, live, +39.2% P&L position the desk says HOLD, whose own "Why this setup" section shows real non-zero pillars (Catalyst 35.3 + Structure 26.5 + Regime 14.7 + Volatility 14.3 ≈ 91), while the SAME brief's "Trade manager read" simultaneously said "Below lane median — #59/59 (score 0, -63 vs median). Leader: GEMI @ 74 — confirm before adding size" — the opposite of what the rest of the brief says about the same position.
- **What changed:** added `HorizonPlay.scoreWithheld?: boolean` (additive, optional), set true in `live-plays.ts` exactly when the `0` fallback fires. `computeLaneRank` now excludes any withheld-score row from the peer pool (so it can't drag down another play's own median/rank either) and returns `null` outright — no "Lane rank" section, no fabricated "#59/59" line — when the CALLING play's own score is withheld, matching the honest-omission pattern this feature's sibling text (`thesisHealthSection`'s "Aggregate score withheld") already uses for the unrelated pillar-aggregate percentage.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief?playId=SWING:AAPL:40` and confirm neither the standalone "Lane rank" section nor "Trade manager read"'s folded rank line appears at all for this position (rather than showing a fabricated last-place rank). Also spot-check any OTHER live committed swing position's brief still shows its normal "Below lane median"/"Top-tier setup"/rank line unaffected — the fix should only suppress the comparison for rows with a genuinely unwired score, never for a real one. If a future closed-population sweep re-pins `feature_vector.evidence_score` onto AAPL's row (or any other legacy row), confirm its lane-rank section starts rendering normally again with no code change needed.

### 327. Ask Largo swing play-brief: "γ vs vanna diverge" used a fixed $0.50 gap, firing on every real ticker regardless of price — fix/vex-vanna-diverge-abs-threshold — 2026-09-21

- **What was broken:** `vexCoaching`'s divergence callout used a fixed absolute $0.50 gap between `gammaFlip`/`vexFlip`, scale-blind across the book's $5-$700+ price range. Live-verified: 0/22 real committed/watch positions with both flips present failed to trigger "diverge" (real gaps ranged 2.06%-61.20% of spot) — the bullet was boilerplate, not a discriminating signal.
- **What changed:** converted the check to percent-of-spot (0.5% threshold, anchored at the original $0.50-at-$100-spot calibration point), and the divergence claim is now silently omitted (not shown) when `spot` is unavailable rather than falling back to the broken absolute check.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>` for a HIGH-priced ticker (e.g. AMD/MSTR/SNOW, spot $300+) with both `gammaFlip` and `vexFlip` resolved and confirm "γ vs vanna diverge" only appears when the gap is a genuinely meaningful fraction of spot (≥0.5%), not on every read regardless of gap size. Also spot-check a LOW-priced ticker (e.g. RUM, GEMI, spot <$10) still gets the divergence callout when its own real gap clears 0.5% of its (small) spot — the fix should not have silenced the bullet everywhere, only the scale-blind false positives.

### 328. Ask Largo swing play-brief: a WATCH candidate matching an existing held position's own ticker+direction was silently treated as "self" — zero concentration warning on the exact "about to double an identical wager" case — fix/book-context-watch-self-exclusion — 2026-09-21

- **What was broken:** `bookContextSection`'s call to `checkPortfolioOverlap` only excludes-self correctly for an already-committed play (via its real `positionId`). A WATCH candidate never carries a ledger `positionId` (only committed plays get one stamped into `play.id`), so the call fell through to the module's DEFAULT `excludeSelfMatch: true` — a fallback scoped by the module's own doc comment to an already-committed play re-reading its own row, not an uncommitted dossier. Net effect: a trader looking at a WATCH signal for a ticker they already hold in the SAME direction saw the existing position silently matched as "self" and got zero concentration warning — the exact "about to double an identical wager" case this section exists to flag. Reduced to a minimal fixture repro (WATCH NVDA LONG candidate vs. an open book already holding NVDA LONG): pre-fix returned `null`, should have returned a Concentration warning.
- **What changed:** `bookContextSection` now passes `{ excludeSelfMatch: false }` whenever there is no resolvable ledger id AND the play is still WATCH — a matching book row in that state can only be a genuine distinct position, never "the candidate re-reading itself." The committed-play (`positionId != null`) branch is unchanged. Also corrected one pre-existing test (`play-brief-intel.test.ts` "a duplicate/rolled row…") that had been asserting the buggy behavior using the fixture's default WATCH status — moved it to `status: "OPEN"`, the scenario it actually meant to describe.
- **RTH check:** once deployed, find a live WATCH candidate whose ticker+direction matches an already-held live position (or wait for one to appear — this is a real, recurring scenario any time a fresh WATCH signal repeats a name already in the book) and pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>` for the WATCH row — confirm "Book context" now renders a Concentration warning naming the existing position, instead of no "Book context" section at all.

### 329. Ask Largo swing play-brief: 9 more sections sampled a fresh `Date.now()` instead of `ctx.readMs` despite `ctx` being in scope — #5351-class readMs-anchor bug, second sweep — fix/swing-playbrief-readms-anchor-sweep — 2026-09-21

- **What was broken:** finding #322's `ctx.readMs` fix (and #323's follow-up) wired 4 of the ~20 sections that independently sample the wall clock for staleness. A fresh grep across `play-brief*.ts` found 9 more call sites where the function already takes `ctx: SwingPlayBriefContext` as a parameter (so `ctx.readMs` was always reachable) but still called `Date.now()` directly: `chartTechnicalsSection`, `preferredGexWalls`, `chartLevelsSection`, `meridianCatalystSection`, `gexPostureSection` (play-brief-intel.ts), `buildStructureLadder` (play-brief-ladder.ts, the Structure Ladder widget), `collectFocalLevels`, `resolveBreakInvalidation`, and **`tradeManagerNarrativeSection`** — the brief's main narrative section (play-brief-narrative.ts). Same risk shape as #322/#323: real sequential I/O between sections means each bare `Date.now()` call can land on a different real instant and disagree with a sibling section about whether the same GEX/Vector/Meridian read is stale.
- **What changed:** mechanical `const readMs = Date.now();` → `const readMs = ctx.readMs ?? Date.now();` (or `ctx?.readMs ?? Date.now()` for `chartTechnicalsSection`'s optional `ctx`) at all 9 sites — a caller without an anchored `ctx.readMs` (any test, or a call path outside `composeSwingPlayBrief`) still falls back to the real wall clock exactly as before. `vectorDeskSection`/`wallDynamicsSection` were checked and correctly left untouched — neither takes `ctx` at its real call site, so this would need a signature change, a genuine narrower follow-up rather than folded into this PR.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>` for a live committed position during RTH and cross-check "Trade manager read" (dealer posture / nearest wall), "GEX posture", "Chart levels", and "Chart technicals" all agree on whether the SAME Vector snapshot / GEX matrix is fresh or stale — no case of one section saying "Last snapshot ... may lag" while a sibling renders the identical underlying data as live. Also check "Meridian catalysts" — its own staleness prefix should agree with whatever the "Data freshness" section (#323) says about the Meridian read in the same envelope.

### 330. Ask Largo swing play-brief: `catalystsSection` sampled a fresh `Date.now()` for fundamentals/news staleness instead of `ctx.readMs` — not among the 9 sites #329/#5392 fixed — fix/swing-catalystssection-readms-anchor — 2026-09-21

- **What was broken:** #329's sweep fixed 9 sections but did not include `catalystsSection` — it read two freshness-bearing fields (`arsenal.fundamentals.as_of`, `arsenal.news.as_of`) with two bare `Date.now()` samples, and unlike the other 9 sections it didn't even take `ctx` as a parameter at all (confirmed against #329's own merged diff, which never touches this function).
- **What changed:** threaded an optional `ctx?: SwingPlayBriefContext | null` second parameter (backward-compatible — an existing `catalystsSection(ecosystem)` call still falls back to a fresh `Date.now()`), derived `readMs = ctx?.readMs ?? Date.now()` once, and used it at both internal staleness checks. Updated the real call site in `buildIntelSections` to pass `ctx` through.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>` for a live ticker with both a short-interest fundamentals read and a recent news read, and cross-check "Catalysts"' own staleness disclosures agree with sibling sections (e.g. "Meridian catalysts", "GEX posture") about whether the SAME underlying arsenal snapshot is fresh or stale in that response — no case of "Catalysts" silently rendering a fundamentals/news read as current while a sibling section flags the identical read window as stale.

### 331. Ask Largo swing play-brief: `wallDynamicsSection`/`vectorDeskSection` — the last two sections still sampling a bare `Date.now()` instead of `ctx.readMs` — fix/swing-vector-desk-readms-anchor — 2026-09-21

- **What was broken:** #330's finding explicitly named these two as the remaining open follow-up: `wallDynamicsSection` and `vectorDeskSection` (play-brief-intel.ts) both gate their entire render on `vectorSnapshotStale(vec, readMs, sessionDate)`, but neither took `ctx` as a parameter at all, so each sampled a bare `Date.now()` directly instead of the request-wide `ctx.readMs` anchor. `vectorDeskSection` is the highest-stakes section in this file to get wrong here — it renders the live entry-zone/targets/invalidation/"Watch now" directive block for OPEN/WATCH plays.
- **What changed:** threaded a NEW trailing optional 4th parameter, `ctx?: SwingPlayBriefContext | null`, onto both functions (same backward-compatible pattern as #330's `catalystsSection` fix — every existing 3-arg call/test keeps compiling and behaving identically off a fresh `Date.now()`), derived `readMs = ctx?.readMs ?? Date.now()`, and used it at the one internal `vectorSnapshotStale(...)` call site in each. Updated the shared production call site in `buildIntelSections` to pass `ctx` through on both calls.
- **Scope closure:** re-grepped `play-brief*.ts` for every remaining bare `Date.now()` — the only 4 left are the canonical `ctx.readMs` stamp point itself (`play-brief.ts`), context construction (`play-brief-context.ts`), a failure-fallback `as_of` timestamp unrelated to a staleness comparison (`play-brief-meridian.ts`), and lookback-window math unrelated to staleness (`play-brief-resolve.ts`) — none match this bug's pattern. This closes the #5351/#5392/#5393/#329/#330 readMs-anchor bug class in the swing play-brief family; a future sweep should still re-verify via grep rather than trust this line.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief?playId=SWING:<ticker>` for a live OPEN/WATCH position and cross-check "Wall dynamics" and "Vector desk" agree with sibling sections (GEX posture, Chart levels/technicals, Trade manager read) about whether the SAME Vector snapshot is fresh or stale in that response — no case of "Vector desk" rendering a live entry-zone/targets/"Watch now" directive block while a sibling section flags the identical Vector read as stale, or vice versa.

### 332. Ask Largo swing play-brief: roll-history narrative fabricated "call" whenever a rolled leg's `contract_type` was never recorded — fix/swing-roll-history-null-right-v2 — 2026-09-22

- **What was broken:** `swingRollHistoryLegFromRow` (`play-brief-roll-history.ts`) mapped a rolled leg's `contract_type` via `r.contract_type === "put" ? "P" : "C"` — a genuinely nullable DB column (`db.ts`: `contract_type TEXT`, no `NOT NULL`) collapsed to a fabricated `"C"` (call) whenever it was null/unset, even though `SwingRollHistoryLeg.right` is typed `string | null` specifically to allow "unknown" and `rollHistoryLine`'s `fmtLeg` already has an honest "contract" fallback for exactly this case — dead code since the roll-history disclosure shipped (#4802) because the mapper never emitted `null`.
- **What changed:** `right: r.contract_type === "put" ? "P" : r.contract_type === "call" ? "C" : null` — any unrecognized/missing `contract_type` now maps to `right: null`, letting the existing honest "contract" fallback render instead of a fabricated direction. Zero behavior change for the two real values ("put"/"call") every populated row uses today.
- **RTH check:** this can only be confirmed the next time a real rolled swing chain has a leg with a genuinely unset `contract_type` — a rare, data-dependent condition, not something guaranteed to appear live during any specific session. If/when such a chain's closed brief is pulled (`GET /api/market/swing/play-brief?playId=SWING:<ticker>:<positionId>`), confirm the "Rolled N times" line reads "... to the $X contract" (never a fabricated "$X call"/"$X put") for that specific leg, while every OTHER rolled leg in the same or other chains with a real recorded `contract_type` still reads its correct "put"/"call" word unaffected.

### 333. Ask Largo swing play-brief: three more sites fabricated "call" for a null contract_type — #5401's own finding doc named this as the exact follow-up worth doing — fix/swing-null-contract-type-coercion — 2026-09-22

- **What was broken:** #5401 fixed `play-brief-roll-history.ts`'s `contract_type === "put" ? "P" : "C"` ternary (fabricated "C" for a null/unrecorded contract type) but explicitly left three sibling call sites untouched, reasoning they were "materially lower real-world exposure" because each already short-circuits to null when expiry/strike are missing. That reasoning didn't hold: `contract_expiry`/`contract_strike`/`contract_type` are three independent nullable columns, so a row can have expiry+strike populated while contract_type alone is null. Found via grep across `src/lib/swing/*.ts`: `closed-plays.ts:96` (`closedDeckSourceFromRow`, feeds the CLOSED-tab contract label), `live-plays.ts:130` (`contractFromRow`, feeds every live OPEN/HOLD/TRIM position's rendered right), and `play-brief-resolve.ts:143` (`rowContractMatches`) — the worst of the three, since it's an IDENTITY-resolution helper (the file's own header: "Fixes ticker-collision bugs e.g. NRG OPEN 110C vs WATCH 115C"), not just a display fabrication: a row of unknown contract type could FALSELY MATCH a caller's `right: "C"` hint, potentially resolving the whole brief to the wrong live position on a multi-contract ticker.
- **What changed:** `closed-plays.ts`/`live-plays.ts` now return `null` ("not reconstructible") for an unrecognized/null `contract_type`, matching each function's own existing pattern for missing expiry/strike. `play-brief-resolve.ts`'s `rowContractMatches` now derives `rowRight` via a new shared pure helper (`rightFromContractType`, added to `play-brief-resolve-pure.ts` — deliberately kept in that heavy-import-free module so it stays unit-testable) that maps null/unrecognized to `null`, so an unknown-type row now fails a specific-right match instead of false-matching "C".
- **RTH check:** once deployed, spot-check a few live CLOSED/OPEN/HOLD/TRIM swing positions' play-briefs render their contract right normally (no regression on the populated-contract_type case, which is every real position written by the current commit/roll-plan code paths). No specific live repro exists for the null case itself (both known writers always populate contract_type today) — this is a defense against legacy/backfilled rows and any future write path that doesn't set it, so there is no "before" state to re-check live; the RTH check here is a negative one (confirm nothing broke for the normal case).

### 334. Ask Largo fresh-angle sweep (HELIX, not swing): print-list TABLE renderer mislabeled every real PUT as a call — fix/helix-print-table-lowercase-option-type — 2026-09-22

- **What was broken:** `formatHelixPrintTable` (`src/lib/bie/dynamic-format.ts`) compared real, always-uppercase `option_type` ("CALL"/"PUT") against the lowercase literal `"put"` — always false — so every table-shaped HELIX print-list answer (triggered by any question matching `wantsHelixPrintList`, e.g. "top 5 prints by premium on NVDA") rendered every print as a lowercase "c" (call), regardless of true side or size. The identical bug was already found and fixed in this same file's PROSE sibling (`helix-read.ts`, via `optionSideSuffix`) — this TABLE path was the missed sibling.
- **What changed:** `formatHelixPrintTable` now calls the same shared `optionSideSuffix` helper (case-insensitive, tolerant of a bare "C"/"P", returns "?" for a genuinely unknown side) instead of the raw broken comparison.
- **RTH check:** during RTH, ask Largo a question that forces the table shape (e.g. "top 5 prints by premium on <a liquid ticker with real put flow>") and confirm at least one row that is genuinely a PUT (cross-check via `GET /api/market/flow-tape` or the Helix desk itself) renders with a lowercase "p" suffix in the table, not "c" — and that a real CALL row still renders "c". Also re-ask the same question WITHOUT the "top N"/"by premium" phrasing (forcing the prose path) and confirm the two renderings agree on every print's side — before this fix they could legitimately disagree for the exact same tape.

### 335. Vector Pulse "flow-print" signal never fired on real production data — uppercase/lowercase option_type mismatch, same bug class as #334/#5405 — fix/vector-pulse-uppercase-option-type — 2026-09-22

- **What was broken:** `flowAlertToPulseSignal` (`src/features/vector/lib/vector-pulse.ts`) compared `flow.option_type` against the lowercase literals `"call"`/`"put"`, but every real producer of that field (`fetchRecentFlows` in db.ts, `parseUwFlowAlert`/`parseOccSymbol` in unusual-whales.ts) emits it UPPERCASE ("CALL"/"PUT"/"UNKNOWN"), and `VectorPulse.tsx`'s only feed for this function is the live `/api/market/flows` route on that exact path. Both `isBullish`/`isBearish` were therefore always false for every real print, so the function's own guard silently dropped 100% of real flow prints — the Vector Pulse "flow-print" signal kind has never fired on real production data since it was built. The file's own tests never caught it because every fixture used lowercase option_type, masking it the same way #334/#5405's HELIX bug was masked the same day.
- **What changed:** normalize `flow.option_type` to uppercase once before comparing (`const side = flow.option_type?.toUpperCase() ?? ""`), used at both the classification site and the render-line suffix. Added two new tests using the real uppercase production shape alongside the existing lowercase fixtures.
- **RTH check:** during RTH, watch the Vector desk's Pulse ticker on a liquid ticker with known large flow (e.g. SPY/SPX/QQQ) and confirm a "flow-print" (💰) line actually appears for a real ≥$1M sweep/block print — cross-check against the same print on `/flows` or the Helix tape to confirm the bull/bear tone and strike/side suffix (C/P) match the real print's true side. Before this fix, no flow-print signal could ever have appeared regardless of real flow volume, so ANY confirmed appearance is itself the validation.

### 336. Ask Largo swing play-brief: meridian/meridianPeer sequenced BEFORE the context fan-out instead of racing inside it, ~tripling worst-case source-timeout budget — fix/swing-brief-meridian-latency-serialization — 2026-09-22
...
- **What changed:** `sectionForSwingPlay` now checks a new `entryWindowExpired` observable immediately after the existing INVALIDATED check — before setup maturity, floor gate, or entry stance — routing an expired pre-entry name to RESEARCH regardless of what its price-vs-trigger geometry happens to read (never applies to a live position). The check reuses (never re-derives) the exact `evaluateSwingEntryEnterability(...).expired` computation the command-deck adapter already runs, fed the identical inputs, so the board's routing and the client pill can never disagree. `buildSwingSections` also stamps the row `watchEntryExpired: true` (new optional `HorizonPlay` field) whenever this is why it moved, reusing the same field name `entry-enterability.ts`'s `deadPlayReason()` already keys on.
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` during RTH and confirm no row in `sections.WATCH`/`sections.WAITING_FOR_ENTRY`/`sections.COMMIT_NOW` carries a `firstSeenAt`/`committedAt` older than its own sub-lane's entry-validity window (TACTICAL 2 / STANDARD 3 / EXTENDED 5 trading days) — any name that stale should now appear in `sections.RESEARCH` with `watchEntryExpired: true` instead. Specifically re-check META (or whichever name is currently the oldest real WATCH candidate) to confirm it either re-qualified fresh or has moved out of WATCH.

### 338. `GET /api/market/swing/record`'s `summary.opens` (and the member-facing "Open" tile) was structurally blind to every Engine B (Banger) open position — fix/swing-record-opens-banger-blind — 2026-09-23

- **What was broken:** `summary.opens` was computed purely from `swing_positions` chains (`fetchSwingPositionsRange`/`fetchSwingPositionChain` query `swing_positions` exclusively — confirmed by direct SQL inspection). Engine B (Banger) open positions live in a completely separate table, `banger_positions`, merged into the Swing lane's board display only by `banger-lane-merge.ts` — never into anything `record.ts`/`record/route.ts` reads. Live repro 2026-09-23: `summary.opens: 3` vs 82 real committed SWING-lane positions (81 banger-origin) on `GET /api/market/nighthawk/horizons?view=swings` — a ~27x undercount, rendered directly to members via `SwingAnalyticsPanel.tsx`'s "Open" tile.
- **What changed:** the route now additionally calls `fetchBangerOpenCount()` (the true-count accessor, gated by `isBangerEngineEnabled()`, fail-soft) and folds it into `summary.opens`; `SwingRecordSummary` gained additive `nativeOpens`/`bangerOpens` fields (Largo Product Contract: wrap, don't flatten) so the native swing-only count stays inspectable. `buildSwingRecordSummary` itself is unchanged (pure, single-caller).
- **RTH check:** once deployed, pull `GET /api/market/swing/record?days=7` and confirm `summary.opens` is close to the real committed count on `GET /api/market/nighthawk/horizons?view=swings` (native + banger, not just the ~3-5 native swing_positions opens) — and that `summary.bangerOpens` is nonzero whenever banger-origin positions are live on the board. Also spot-check the Swings desk's "Open" tile in the UI itself now reads a realistic number instead of a single-digit undercount.

### 339. `fetchBangerOpenBookRows`'s hardcoded `limit=80` silently truncated the real open banger book across 5 consumers (board, live marks, identity resolution, concentration check) — fix/banger-open-book-limit-truncation — 2026-09-23

- **What was broken:** every call site of `fetchBangerOpenBookRows` hardcoded (or defaulted to) `limit=80`, so once real open banger positions exceeded 80 the query silently dropped the OLDEST rows (`ORDER BY session_date DESC, id DESC`). Live repro 2026-09-23: `fetchBangerOpenCount()` (true count) reported 168 real open positions while `GET /api/market/nighthawk/horizons?view=swings`'s `committed` array (fed by the truncated read) showed only 85 total — roughly half the real open book invisible to members, Largo, live-mark freshness tracking, and the play-brief identity resolver.
- **What changed:** `fetchBangerOpenBookRows(limit?: number)` — `limit` is now optional and the SQL carries no `LIMIT` clause at all when omitted, so the real open book is never truncated. All 5 previously-hardcoded call sites (`horizons/route.ts`, `banger/board/route.ts`, `play-brief-resolve.ts`, `live-marks-active.ts`, `play-brief-context.ts`) now call it with no argument.
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` during RTH and confirm `board.lanes.SWING.committed.length` (plus `GET /api/market/swing/record?days=1`'s `summary.bangerOpens`) tracks the TRUE open count rather than plateauing near 80-85 as the real open population grows past that threshold again. Also spot-check that a banger-origin position committed a while ago (older `firstSeenAt`) still appears on the board and in a play-brief lookup, not just the most recently opened ones.

### 340. Expired Banger (Engine B) positions never closed — 41/168 (24%) of the open book was zombie-stuck, one 40 days past expiry — fix/banger-expired-position-zombie-close — 2026-09-23

- **What was broken:** `runBangerLiveSync` (`live-sync.ts`) skipped (`noQuote += 1; continue;`) any row with no live option mark, with no expiry-aware force-close path anywhere — once a contract's expiry passed, the provider simply never quotes it again, so the row sat in `OPEN`/`PARTIAL` forever. Live measured 2026-09-23: 41 of 168 open banger rows (24%) had an already-expired contract (3 `OPEN`, 38 `PARTIAL`); the oldest (FLY, id 99) expired 2026-08-14 — 40 calendar days stuck open, never graded.
- **What changed:** added an optional `fetchExpiryClose(ticker, expiryYmd)` dependency; when a no-quote row's contract `isExpirySettled` (`expiry-liveness.ts`), it's force-closed via a new `settleExpiredBangerRow` helper at OCC intrinsic value (`max(0, underlyingCloseOnExpiryDate - strike)`, every banger contract being a long call), reusing the existing EXIT_RUNNER/STOP_OUT realized-P&L formula, status `CLOSED_RUNNER`, `scaleOutAction: "EXPIRED"`. Wired the real close fetch in `banger-live-sync/route.ts` via the already-existing `fetchOpenClose` (Polygon `/v1/open-close`). No schema change; omitting the new dep (or a `null` close) reproduces the old skip behavior exactly.
- **RTH check:** once deployed and the `banger-live-sync` cron has run a few RTH ticks, pull `GET /api/market/banger/board` and confirm the previously-identified stuck tickers (FLY id 99, KTOS id 88, UUUU id 96, RCAT id 97, CRCG ids 190/457, CRCA id 197, ARX id 252, CELH id 430, ASST ids 454/516, CRM id 474, FBTC id 492, ETH id 513, SMMT id 678, AUR id 702, INSP id 878, ACVA id 884, SMST id 892, DNA id 1120, RBLU id 1122) have moved to `closed[]` with `status: CLOSED_RUNNER` and a `scale_out_reason` naming "expired — settled at intrinsic value". Also confirm `GET /api/market/swing/record`'s `summary.bangerOpens` has dropped by roughly that count (168 → ~127) and that `board.lanes.SWING.committed.length` on `horizons?view=swings` now more closely tracks the true remaining open population.

### 341. `fetchBangerClosedBoardRows` sorted by entry-time recency, not close-time recency — hid freshly-closed old positions — fix/banger-closed-board-sort-order — 2026-09-23

- **What was broken:** `ORDER BY session_date DESC, id DESC` sorts by when a position was OPENED, never updated after entry — despite the function's own doc comment claiming "newest first" means most recently CLOSED. Discovered live while verifying #340's fix: once #5468 deployed and the live-sync cron's first RTH tick closed 45 real rows (`fetchBangerOpenCount()` 168→123, confirmed via the unpaged count), none of the 19 previously-stuck tickers from #340 appeared in `GET /api/market/banger/board`'s `closed[]` 60-row window — every visible row had a September `session_date` (this week's entries), while the freshly-closed July/August-entered zombie positions ranked below the cutoff regardless of how recently they actually closed. Printed `id, session_date, closed_at` for the full window and confirmed `closed_at` values were scattered non-monotonically, proving the ordering ignores close time entirely.
- **What changed:** `ORDER BY` changed to `closed_at DESC, id DESC`. `updateBangerLiveState` already stamps `closed_at` on every CLOSED_RUNNER/STOPPED transition (both the normal live-exit path and #5468's expiry-settlement path), so no backfill/migration needed — pure sort-order fix, one call site (`banger/board/route.ts`).
- **RTH check:** once deployed, pull `GET /api/market/banger/board` and re-run #340's own re-verify: confirm the 19 previously-stuck tickers (FLY id 99, KTOS id 88, UUUU id 96, RCAT id 97, CRCG ids 190/457, CRCA id 197, ARX id 252, CELH id 430, ASST ids 454/516, CRM id 474, FBTC id 492, ETH id 513, SMMT id 678, AUR id 702, INSP id 878, ACVA id 884, SMST id 892, DNA id 1120, RBLU id 1122) now actually APPEAR in `closed[]` with `status: CLOSED_RUNNER` and `scale_out_reason` containing "expired — settled at intrinsic value" — #340's own check could not be completed before this fix because the sort bug hid them regardless of #5468 working correctly. Also spot-check that `closed[]` now genuinely reads as "recently closed" (timestamps monotonically decreasing down the list) rather than jumping around.

### 342. `horizons` route's `troughPremium` rounded at 2dp while its sibling `mid`/`peakPremium` were 4dp — a real trough could read above the current mark — fix/horizons-trough-premium-rounding-precision — 2026-09-23

- **What was broken:** the route's `roundFloats(...)` `keyDp` override map covered `mid`/`entryPremium`/`peakPremium`/`gamma`/`theta`/`vega`/`iv` but never `troughPremium`, so it fell through to the plain 2dp default while its siblings stayed at 4dp. For sub-$1 Banger-origin contracts that mismatch is large enough to invert the displayed relationship: a real trough tick rounds up past a mid/peak that stayed essentially unrounded, producing a trough that reads HIGHER than the current mark — definitionally impossible for the least-observed value. Live measured: 6 of 78 open Banger-origin positions on the board carried this exact violation (BKKT, REPL, SLS, DKS, MRNA, ARKG).
- **What changed:** added `troughPremium: 4` to the same `keyDp` map. One-line fix, no schema/data change — the underlying `trough_premium` column and its `LEAST(...)` latch were already correct.
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` and confirm every committed Banger-origin play satisfies `troughPremium <= contract.mid <= peakPremium` (with small float tolerance) — spot-check BKKT/REPL/SLS/DKS/MRNA/ARKG specifically, since those are the 6 confirmed-live violations before this fix.

### 343. Swing play-brief `followups[]` chips were identical for every OPEN play regardless of state — feat/swing-situational-followups — 2026-09-23

- **What was broken:** `followupsFor(play)` built its five suggestion chips purely from the open/watch/closed status bucket with the ticker substituted in, never consulting any situational signal — even though `play.manageAction` (the live manage-engine verdict) is already computed and already rendered a few lines earlier in the same Management section. Live spot-check across 6 sampled committed positions: SKHY (-48.4%, HOLD), BTDR (-59.5%, sitting near its hard stop) and RBRK (+124.4%, TAKE_PARTIAL already firing) all returned byte-identical `followups[]`.
- **What changed:** added a small `manageAction` → targeted-phrase lookup (TAKE_PARTIAL → "Should I trim more?", EXIT_RUNNER → "Why exit the runner now?", STOP_OUT → "Why is this at its stop?"). One extra chip is appended only when the OPEN play carries a mapped manage action; the existing five generic chips are untouched, and a plain HOLD (the common case) is unaffected.
- **RTH check:** once deployed, pull `GET /api/market/swing/play-brief` for a position currently showing `manageAction: "TAKE_PARTIAL"` (or `STOP_OUT`) on `GET /api/market/nighthawk/horizons?view=swings` and confirm `envelope.followups` carries the new situational chip alongside the original five, while a plain-HOLD position's followups are unchanged.

### 344. Situational followups (#343 above) covered only Banger-lineage manage actions, missing native EXIT/ADD — fix/swing-followups-exit-add-coverage — 2026-09-23

- **What was broken:** #343's `SITUATIONAL_FOLLOWUP_BY_MANAGE_ACTION` mapped only the three `ScaleOutAction` values (TAKE_PARTIAL/EXIT_RUNNER/STOP_OUT — Banger-lineage). `SwingManageAction` (`manage.ts`) is `ScaleOutAction | "EXIT" | "ADD"` — EXIT and ADD are the native `swing_positions` verdicts (broken-thesis full close, advisory add), neither covered, so a native position with a broken thesis got no situational chip at all. Found by reading the type after spot-checking #343's live deploy, not a live repro (no position was in EXIT/ADD state at spot-check time).
- **What changed:** added `EXIT: "Why exit now?"` and `ADD: "Should I add to this?"` to the same lookup map.
- **RTH check:** once deployed, watch for a native position (no Banger origin) whose `manageAction` reads `EXIT` (broken thesis) on `GET /api/market/nighthawk/horizons?view=swings`, and confirm its `GET /api/market/swing/play-brief` `envelope.followups` now carries "Why exit now? (<ticker>)".

### 345. `board.lanes.SWING.watch` back-compat array served entry-window-expired names PR #337 was supposed to demote — fix/swing-watch-backcompat-entry-window-expired — 2026-09-23

- **What was broken:** PR #337 (2026-09-22) taught `sectionForSwingPlay` to route a pre-entry name past its own entry-validity deadline to `sections.RESEARCH` instead of `sections.WATCH`. That fix only touched the `sections` grouping — `assembleSwingServingLane`'s separate back-compat `watch` field (the flat array `GET /api/market/nighthawk/horizons?view=swings` actually serves as `board.lanes.SWING.watch`) was independently derived via `plays.filter((p) => p.status === "WATCH")`, never consulting the router at all, so it kept serving the stale name anyway. Live repro: AMZN, TACTICAL sub-lane (2-day entry window), first flagged 15 days earlier, still in `board.lanes.SWING.watch` while its own play-brief's Entry section correctly said "Serving section: RESEARCH... entry-validity window expired" — two surfaces backed by the same data disagreeing on whether the same name is live or dead.
- **What changed:** `assembleSwingServingLane` now derives `watch` as `sections.WATCH` directly (computing `sections` once, reusing it) instead of independently re-filtering raw status — the two can no longer disagree by construction. `committed` is unchanged (no live-position entry-window-expiry case exists).
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` and confirm every name in `board.lanes.SWING.watch` also appears in `board.lanes.SWING.sections.WATCH` (never `sections.RESEARCH`) — re-check AMZN specifically (or whichever name is currently the oldest real WATCH candidate) to confirm it has moved out of the back-compat `watch[]` array if its entry window has expired.

### 346. `board.lanes.SWING.committed`/`committedCount` back-compat overcounted unclassified/FORMING plays the router itself sends to WATCH or RESEARCH — fix/swing-committed-count-gate-blocked-overcount — 2026-09-25

- **What was broken:** The exact same bug class as #345 above, just on the COMMIT side, which #345's own fix left untouched on the stated (never re-verified) belief that "no live-position entry-window-expiry case exists for it." `assembleSwingServingLane`'s `committed` field was derived via `plays.filter((p) => p.status === "COMMIT")` — the mechanical floor-gate result, not the router's verdict. A pre-entry play with no setup-maturity read (unclassified) or one still `FORMING` its thesis can carry `status:"COMMIT"` while the router correctly places it in `sections.WATCH`/`sections.RESEARCH`, never `COMMIT_NOW`/`WAITING_FOR_ENTRY`. Live repro (non-RTH, ~00:19 ET): `board.lanes.SWING.committedCount` read 91 while `GET /api/market/swing/record`'s `summary.opens` (real tracked positions) was 82 — 9 gate-blocked/no-`positionId` theses double-counted (6 of them byte-identical duplicates also present in `watch`).
- **What changed:** `committed` now derives from the already-computed `sections` object — `COMMIT_NOW ∪ WAITING_FOR_ENTRY ∪ MANAGING ∪ SCALING_OUT ∪ EXITING` (everything except `WATCH`/`RESEARCH`) — instead of independently re-filtering raw status, matching the already-documented `committedCount` semantics two existing `product-reads.ts` regression tests lock in (a gate-blocked-but-floor-cleared thesis legitimately stays counted, disambiguated separately via `open_position_count`/`commit_gate_blocked` at the Largo layer).
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` and `GET /api/market/swing/record` together and confirm `board.lanes.SWING.committedCount` no longer exceeds `record.summary.opens` by more than the count of genuine pre-entry `COMMIT_NOW`/`WAITING_FOR_ENTRY` candidates currently on the board (i.e. every name in `board.lanes.SWING.committed` also appears in `sections.COMMIT_NOW`/`WAITING_FOR_ENTRY`/`MANAGING`/`SCALING_OUT`/`EXITING`, never `sections.WATCH`/`sections.RESEARCH`). **Confirmed live 2026-09-25 ~05:19 UTC**, deployed and holding: `committedCount=82`, matching `record.summary.opens` exactly (ECS task def `blackout-production-web:1789`, image tag `0238c3f13...`).

### 347. `portfolio/sector-map.ts` had no quantum-computing cluster, so `checkPortfolioOverlap` missed a real concurrent-position concentration (RGTI+QBTS+IONQ) — fix/sector-map-quantum-computing-cluster — 2026-09-25

- **What was broken:** `sectorFor`/`SECTORS` is the single curated ticker→theme map both the allocation engine and the swing engine's `theme-cluster.ts` (feeding `checkPortfolioOverlap`'s Book-context concentration evidence) resolve against; an unmapped ticker becomes its own isolated cluster by design. `RGTI` (Rigetti), `QBTS` (D-Wave), `IONQ` (IonQ), and `QUBT` (Quantum Computing Inc) had no entry at all. Live repro: the committed book held RGTI+QBTS+IONQ concurrently LONG (same TACTICAL/BREAKOUT discovery batch), and RGTI's live play-brief Book-context section made zero mention of the other two — a genuinely correlated 3-name basket read as no concentration. Same gap shape as the 2026-09-20 crypto-equity extension.
- **What changed:** added `"quantum-computing": ["RGTI", "QBTS", "IONQ", "QUBT"]` to `SECTORS`, deliberately conservative (only unambiguous liquid pure-plays), matching the crypto-equity precedent's own stated discipline.
- **RTH check:** once deployed, pull a live play-brief for any of RGTI/QBTS/IONQ/QUBT while 2+ of them are concurrently open/committed same-direction, and confirm the Book-context section now names the others as a shared `quantum-computing` concentration instead of omitting them.

### 348. `portfolio/sector-map.ts`'s `semis` cluster had no ALAB/CRDO/KLAC, so `checkPortfolioOverlap` missed a real concurrent-position semis/AI-infra concentration — fix/sector-map-semis-alab-crdo-klac — 2026-09-25

- **What was broken:** Same gap shape as #347 above (quantum-computing) and the 2026-09-20 crypto-equity extension: an unmapped ticker becomes its own isolated cluster by design, so a real correlated name reads as "no concentration." `ALAB` (Astera Labs) and `CRDO` (Credo Technology) are AI-datacenter connectivity-chip plays (PCIe/CXL retimers, SerDes/optical DSP) — same risk bucket as the already-listed NVDA/AVGO/MRVL. `KLAC` (KLA Corp) is semis-equipment/process-control — same subsector as the already-listed LRCX/AMAT. None had a `semis` entry. Live repro: the committed book held ALAB concurrently LONG across 3 positions, CRDO across 2, plus KLAC/LRCX/INTC — a real 6-position semis/AI-infra concentration — and ALAB's/CRDO's live play-briefs' Book-context sections reported only same-ticker concentration, never the cross-ticker semis basket.
- **What changed:** added `"ALAB", "CRDO", "KLAC"` to `SECTORS.semis`. Left out SMTC/AMBA/AXTI (lower confidence, not live-verified this pass) per the same deliberately-conservative discipline the crypto-equity/quantum-computing precedents state.
- **RTH check:** **CONFIRMED LIVE 2026-09-25 ~09:04 UTC**, deployed and holding (ECS task def `blackout-production-web:1792`, all 8 tasks cycled onto it, old `:1791` fully drained). `GET /api/market/swing/play-brief?playId=SWING:ALAB&ticker=ALAB`'s **Book context** section now reads: *"already holding 8 same-direction positions in theme 'semis': SOXL LONG, INTC LONG, CRDO LONG (position #1275), KLAC LONG, LRCX LONG, ALAB LONG (position #1153), ALAB LONG (position #1139), CRDO LONG (position #1136)"* — the fix is live and correctly reports the full cross-ticker concentration, not just same-ticker.

### 349. `researchGateBlocks` (swing entry-verdict) had no case for a lapsed entry window, so a real triggered setup read as a rejected thesis — fix/swing-research-block-entry-window-expired-reason — 2026-09-25

- **What was broken:** `sectionForSwingPlay` routes a pre-entry play to `RESEARCH` for four reasons (persistence gap, unclassified, `INVALIDATED`, or — per a 2026-09-12 fix — `entryWindowExpired`, a real triggered setup whose 2-5 day clean-fill window lapsed). `researchGateBlocks` (the function supplying the member-facing `gateBlocks` reason/`recNote` for every RESEARCH row, read by both the Command Deck and Ask Largo's play-brief) had explicit cases for the first three but never checked `entryWindowExpired` — the 2026-09-12 fix wired that signal into the WATCH-pill display only, never into this sibling consumer. Live repro: AMD — score 81.3, `TRIGGERED`+`AT_TRIGGER`, real pillar factors (Rel. strength 31.1, Regime 22.2, sector-leadership SMH +17.77pp), first seen 2026-07-30 (57 days prior) — fell through to the generic `"Desk is passing this name — thesis needs more work before it can be served"` instead of the real reason.
- **What changed:** added `entryWindowExpired?: boolean | null` to `SwingEntryVerdictInput`; added a `researchGateBlocks` branch returning `entry_window_expired` with an honest "triggered but the window lapsed" reason; threaded `adapters.ts`'s already-computed `watchEntryExpired` through the `swingEntryVerdict(...)` call.
- **RTH check:** once deployed, pull `GET /api/market/nighthawk/horizons?view=swings` for a name in `sections.RESEARCH` with `watchEntryExpired: true` and confirm its Ask Largo play-brief / Command Deck entry verdict shows the `entry_window_expired` reason rather than the generic "thesis needs more work" fallback.

### 350. `scoreWithheld` never reached `adapters.ts`/`TerminalPlay` — Command Deck's raw score display had no way to distinguish a withheld `0` from a real one — fix/swing-score-withheld-terminal-play-thread — 2026-09-25

- **What was broken:** `live-plays.ts` computes `HorizonPlay.scoreWithheld` (`score:0` is a fallback, not a real measured value, when `feature_vector.evidence_score` is missing at read time) but only `play-brief-lane-rank.ts` ever read it (2026-09-21 fix, live repro `SWING:AAPL:40`). `adapters.ts`'s `terminalPlayFromHorizon` — the one shared SWING/LEAPS builder feeding both the Command Deck and Ask Largo's play-brief — never received the flag. Live repro: `SWING:AAPL:40` briefly showed `score:0` on the board while its "Why this setup" pillars summed to `75.9` (a direct violation of `live-plays.test.ts:603`'s own tested invariant); re-checked ~45 minutes later (independently cross-verified by a concurrent Claude session) the row had self-healed — confirming a real but transient staleness window, not a permanent bad row.
- **What changed:** added `scoreWithheld?: boolean` to `HorizonDeckSource`/`TerminalPlay`, threaded it through `terminalPlayFromHorizon` and both call sites that build a `HorizonDeckSource` (`containers.tsx`, `play-brief-resolve.ts`). Guarded `PlayTerminal.tsx`'s two raw `play.score` displays: the header score badge shows `"—"` instead of a misleading `0` when withheld; the empty-factors fallback distinguishes "withheld" from the pre-existing "not served for this lane yet" case.
- **RTH check:** **CONFIRMED DEPLOYED 2026-09-25 ~11:40 UTC** (ECS task def `blackout-production-web:1793`, 8/8 tasks). `scoreWithheld:true` still correctly served live for `SWING:AAPL:40`. The client-side render guard (header badge / empty-factors fallback) is covered by the RED→GREEN unit test rather than a live browser check, since this specific row is currently in its healed state (score/factors already both zeroed, so the guard has nothing to visibly suppress right now) — re-check with a live screenshot if/when a fresh `scoreWithheld:true` + non-trivial-score row is caught in the act.

### 351. `vector-full-state-snapshot` cron warmed only the static Vector allowlist, never a real committed swing position outside it — fix/vector-full-state-warm-swing-positions — 2026-09-26
