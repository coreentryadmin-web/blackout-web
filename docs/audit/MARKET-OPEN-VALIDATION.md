...
### 111. Production Next.js had an unauthenticated RCE (GHSA-2xp9-vwfh-vxw4) — patched 8 other npm-audit findings same PR — fix/npm-audit-next-critical-rce-patch — 2026-09-11
...
### 112. Vector background crons blocked the shared web ECS event loop, causing ALB p99 25-50s tail-latency spikes during RTH — fix/vector-background-cron-event-loop — 2026-09-11
...
### 113. Swing peak/trough premium watermarks were never bracketed around the REALIZED exit — command-deck card understated the true excursion on closed positions — fix/swing-watermark-bracket-at-exit — 2026-09-11

- **What was broken (live repro 2026-09-11 — AAPL positionId 36 showed trough -30.5% while its actual realized exit was -56.2%; INTC/MSTR/CCI/IGV showed the same shape the same window):** `peak_premium`/`trough_premium` are latched ONLY during RTH, on a 15-minute cadence (`swing-active-refresh`'s `updateSwingLiveState`) — so they can never capture an overnight/pre-market gap or an intrabar move between polls. A CLOSED position's REALIZED exit could therefore land outside the very `[trough, peak]` range its own command-deck card presents as "the full excursion since entry" (`PlayTerminal.tsx`'s Peak/Trough caption) — understating how bad (or good) the actual outcome was, same bug shape as the entry_premium bracket fixed 2026-08-06 (SEV-2), just at the other end of the position's life.
- **What changed:** Widen-only bracket (`GREATEST`/`LEAST`, never narrows a genuinely-ratcheted extreme) applied at TWO points: (1) a one-time idempotent `ensureSchema` migration backfills every already-graded row whose bracket doesn't yet contain its realized exit (safe to re-run on every boot — no-op once converged); (2) `gradeSwingPosition` now performs the same widening AT GRADE TIME (mirrored in a new pure `swingGradeWatermarkBracket` helper, unit-tested) so newly-closed rows never regress into this state going forward. Reviewed independently (clear root cause, correct discipline match to the prior entry_premium fix, idempotent migration) before merge.
- **RTH check:** Pull `GET /api/market/swing/record` for any position that closes tomorrow and confirm `peak_premium`/`trough_premium` on the closed leg already bracket the realized `exit_mark`/`realized_pnl_pct` (no post-hoc correction needed) — and spot-check the command-deck card's Peak/Trough caption for that same position renders the corrected, bracketed range.
