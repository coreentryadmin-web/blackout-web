...
### 31. Largo `get_technicals` — live WS price with null `change_pct` — fix/largo-technicals-ws-change-pct-rebase — 2026-09-05

- **What was broken:** `buildLargoTechnicals()` used Polygon WS price when fresher but left `change_pct: null`, unlike quote/positioning paths that rebase against the REST snapshot baseline.
- **What changed:** Index WS path calls `rebaseChangePct(ws, quoteRaw[sym])`; equity WS path prefers `wsCandle.changePct` then rebases against prefetched REST snapshot.
- **RTH check:** Ask Largo for technicals on NVDA/SPX during RTH — `change_pct` must be present (not null) when REST baseline exists and WS price is live.
