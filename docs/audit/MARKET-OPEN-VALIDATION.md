...
### 61. Ask Largo swing brief — stale Vector still coached confluence/VEX/magnet/walls — fix/largo-stale-vector-coaching-gate — 2026-09-06

- **What was broken:** `collectCoachingBullets()` pushed Vector-sourced coaching (VEX, confluence, expected move, wall integrity, magnet, wall dynamics, technicals, vector play) even when `vectorSnapshotStale()` was true — only `technicalsCoaching` and `crossDeskCoaching` gated internally.
- **What changed:** Gate the entire Vector coaching block in `collectCoachingBullets()` behind `vectorLive = !vectorSnapshotStale(vec, readMs)`.
- **RTH check:** Open Ask Largo on a swing with stale Vector snapshot (>120s) that still has confluence zones / expected-move bands in cache — confirm "Trade manager read" bullets omit Confluence, Expected move, VEX, Vector desk, and VWAP/chart-read coaching; non-Vector coaching (trim ladder, gates, etc.) still renders.
