...
- **What was broken:** `sector-map.ts`/`theme-cluster.ts` (the shared theme resolvers concentration detection reads) had no entry for `MSTU`/`MSTX` (leveraged ETFs on `MSTR` itself), `GLXY` (a crypto-holdings proxy), or `XRP`/`XRPZ`/`XXRP` (three wrapper tickers on the same token) — each resolved to its own isolated cluster and could never trip a Book Context concentration flag against the position it duplicates. Live: the book held `MSTU`+`MSTX` simultaneously and a 6-way instance (`XRP`/`XRPZ`/`XXRP`/`GLXY`/`MSTU`/`MSTX`, all same-direction LONG) with zero flag between any pair.
- **What changed:** added `MSTU`/`MSTX`/`GLXY` to `crypto-equity`; added `XRP`/`XRPZ`/`XXRP` to `ETF_PROXY_THEMES` under a new, deliberately separate `"crypto-xrp"` cluster (a token's own price is a different risk driver than mining/holding-equity beta).
- **RTH check:** pull a live swing play-brief for any ticker in the book alongside another MSTR-linked leveraged ETF or another XRP wrapper, and confirm Book Context now cites the concentration.
