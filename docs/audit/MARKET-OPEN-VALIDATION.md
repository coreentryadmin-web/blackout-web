...
- **Note:** #110 (roll-history "contract" vs "put"/"call" fix) is in-flight on a separate PR not yet merged to `main` as of this entry — numbered #111 to avoid colliding with it once it lands; ordering in this file may end up non-sequential where two lanes' PRs merge out of creation order, which is expected/accepted per this file's own cross-PR-ordering guidance.
