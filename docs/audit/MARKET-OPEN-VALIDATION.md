...
### 49. Ask Largo swing brief — thesis-health inputs not wired on committed rows — fix/swing-brief-thesis-inputs-wire — 2026-09-06

- **What was broken:** `attachThesisExplanation` restored factors/regime on the play-brief path but not `setupState`/`entryStatus`/`signalKinds`, so `computeSwingThesisHealth` used default pillar labels (`unknown`/`n/a`/`no signals`) and every committed swing showed the generic ~46% uncalibrated band even with a matching dossier.
- **What changed:** `attachPlayBriefThesisInputs` (brief-only) wires ledger levels + play contract + commit archetype signal path; used in `loadOpenTerminalPlay` and WATCH lane resolution. Main `getSwingServingLane` unchanged.
- **RTH check:** Open Ask Largo on a committed SWING with a matching dossier — thesis health should show calibrated pillar labels (not generic 46% band); regime pillar not `unread`; Verdict/Management may now show a real calibrated % when inputs are grounded.
