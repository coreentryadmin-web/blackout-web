...
- **Still open — a genuine RTH prediction, not yet verified:** CG (`SWING:CG:25`, TAKE_PARTIAL) sat at +159.6% P&L (well past its own +100% trim trigger, $5.20) with the ladder's `+100%` rung still showing unfired (no ✓) as of a 2026-09-10 evening (after-hours) read — consistent with `fired` being a flag the market-hours-only `swing-active-refresh` cron only sets when it actually observes the crossing, not something recomputed fresh from live price on every brief render. **RTH check:** once market opens, pull CG's play-brief again and confirm the `+100%` rung now shows `✓` and the Position section carries a "Blended P&L (realized trim + open runner)" line (same shape as CRWD's, banked at the rung's `trigger_pct`, i.e. exactly `+100%` for the fired tranche) — not still unfired hours into RTH, which would indicate a genuine gap in the cron's rung-detection rather than just normal after-hours lag.
...
### 101. `legacy-e2e-healthcheck.mjs` Stage B falsely went RED on a normal pre-market no-live-quote state — fix/legacy-healthcheck-zero-bidask-false-positive — 2026-09-11
