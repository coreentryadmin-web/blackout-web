...
### 0ae. Stocks/polygon WS stall watchdog future guards — fix/ws-stall-watchdog-future-guard (pending)

**What was broken:** `startStocksWatchdog()` and the polygon indices stall watchdog still used raw `Date.now() - timestamp` — clock-skewed future `lastMessageAt` stamps never trigger reconnect (same class as #3760/#3762/#3771).

**Fix:** Route both through `isWsUpdatedAtFresh` from `timestamp-freshness.ts`.

**Check at the open:** Admin System Vitals stocks + indices socket tiles still reconnect on genuine silence; no false-positive stall storms from skewed clocks.
...
### 0ab. Vector GEX walls spot constraint — fix/vector-gex-walls-spot-constraint (merged #3764)
