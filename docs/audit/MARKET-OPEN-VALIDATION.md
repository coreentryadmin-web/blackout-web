...
### 40. Ask Largo swing "brief v4" collapse — silently deleted Book context / Desk context (P1) — fix/swing-collapse-drops-book-context — 2026-09-06

- **What was broken:** #4119's `collapseRedundantIntelSections()` dropped "Book context" and "Desk context" whenever "Trade manager read" was present, assuming their content was folded into the narrative — true against its stale fork point, false against current `main` (#4116 had already removed the narrative's book-concentration bullet; #4111 had already moved only part of desk-context content into the narrative, keeping the rest supplementary). Live-verified on `main`: a theme-overlapping book produced ZERO concentration warning anywhere, with a footnote falsely claiming it was "folded into Trade manager read above." Flagged in a PR comment before merge; merged 23 seconds later anyway on the same stale base (same self-merge pattern as #4110 — see CLAUDE.md's "THE MIRROR FAILURE").
- **What changed:** Removed "Book context" and the stale "Desk consensus" (now "Desk context") from the collapse set; documented why each stays excluded so a future section rename doesn't reintroduce this.
- **RTH check:** Open Ask Largo for an OPEN/HOLD swing play whose ticker shares a theme with an existing open position — confirm the "Book context" section (concentration/conflict warning) still appears even when "Trade manager read" leads. Separately, for a play with NH trade history or a recent flow anomaly on the name, confirm "Desk context" still appears with that content.
