...
| Field | Detail |
|---|---|
| **Symptom** | `/api/market/flows/stream` SSE events could carry IEEE float tails on premiums/GEX levels while the REST `/flows` lane already rounded at the boundary. |
| **Root cause** | `JSON.stringify(payload)` serialized raw flow enrichment numbers without `roundFloats`. |
