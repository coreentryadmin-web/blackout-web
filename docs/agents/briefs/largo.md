...
| Tool registry | `src/lib/largo/registry/capability-registry.ts` — 138 tools, one capability entry each |
...
| Tool count by product (roughly) | 138 tools total across Helix, Thermal, Vector, Meridian, Night Hawk, SPX, plus cross-cutting (`get_cross_product_read`, `get_market_context`, `get_news`, `get_web_search`, etc.) |
...
For each of the 138 tools, record: which product it reads, whether it returns a `ProductRead<T>`
...
Run `scripts/audit/largo-truncation-probe.mjs` against every one of the 138 tools, not just the
...
a hypothesis, not a fix — you own 138 tools through one transport layer, so a defect found in one is
