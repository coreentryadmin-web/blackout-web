...
For each of the 137 tools, record: which product it reads, whether it returns a `ProductRead<T>`
...
Run `scripts/audit/largo-truncation-probe.mjs` against every one of the 137 tools, not just the
...
a hypothesis, not a fix — you own 137 tools through one transport layer, so a defect found in one is
