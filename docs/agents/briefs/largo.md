...
| Tool registry | `src/lib/largo/registry/capability-registry.ts` — 1987 lines, 135 tools, one capability entry each |
...
| Tool count by product (roughly) | 135 tools total across Helix, Thermal, Vector, Meridian, Night Hawk, SPX, plus cross-cutting (`get_cross_product_read`, `get_market_context`, `get_news`, `get_web_search`, etc.) |
...
For each of the 135 tools, record: which product it reads, whether it returns a `ProductRead<T>`
...
Run `scripts/audit/largo-truncation-probe.mjs` against every one of the 135 tools, not just the
...
a hypothesis, not a fix — you own 135 tools through one transport layer, so a defect found in one is
