...
| `lane:<name>` | Which product lane — `helix`, `thermal`, `vector`, `meridian`, `nighthawk`, `spx`, `seo`, `x-content`, `largo`, `ui-ux`, `qa-adversarial`, `growth`, `discovery` |
...
| `qa-adversarial` | Independent red-team QA — second-party validation of every product lane's own "VERIFIED" claim; finds failures, routes them to the coordinator, does not fix product code |
| `growth` | Cross-lane growth controller — not product-scoped. Owns the discovery→trust→signup→activation→paid funnel end to end: cross-channel growth scoreboard, ICE-ranked growth backlog, conversion-funnel diagnosis, growth-loop ideation, competitive intelligence, backlink target curation. Synthesizes and prioritizes across `seo`, `x-content`, `discovery` and `ui-ux` rather than duplicating them; routes execution to the owning lane. Brief: `docs/agents/briefs/growth-controller.md` |
| `discovery` | 24/7 general new-work hunter across the fleet |
