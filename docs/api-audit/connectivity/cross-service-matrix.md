...
---
## Connectivity Matrix � 2026-06-30 02:59 ET
**PASS: 18 | FAIL: 0 | WARN: 1 (off-hours flow staleness � expected, market closed)**

Run context: pre-market (~06:00 ET). Live probes via apex `blackouttrades.com` + Bearer (www strips auth). All 6 service endpoints returned HTTP 200.

### Source ? Consumer matrix
| Channel | Status | Evidence |
|---|---|---|
| SPX ? HEATMAP | PASS | desk.gex_walls derived from same gamma-desk/GEX provider; GEX endpoint `gex_cross_validation` callWall/putWall/flip all match, divergence 1.46 |
| SPX ? LARGO | PASS | `get_spx_structure` / `get_spx_play` / `get_spx_confluence` / `get_gex` all read `getLargoSpxLiveDesk` |
| SPX ? NWATCH | PASS | position-detail.ts uses `loadMergedSpxDesk` + `computeSpxConfluence`; verdict.ts SPX price/levels |
| SPX ? NHAWK | PASS | nighthawk spx-gap / index-dossier consume SPX desk |
| HELIX ? SPX | PASS | SPX desk carries `flow_0dte_net`=434.7M, `tide_bias`=bullish, `spx_flows`=32, `unified_tape`=29 |
| HELIX ? LARGO | PASS | `get_options_flow` returns spx desk tape ("same feed as dashboard") + `get_flow_tape` / `get_global_flow` |
| HELIX ? NHAWK | PASS | candidates.ts `aggregateTickerFlows` + `flow-streak` over flow_alerts; LIVE play thesis cites "\.4M across 80 alerts, 4-day streak" |
| HELIX ? NWATCH | PASS | verdict.ts flow signals (FLOW_MIN_PREMIUM 250k, FLOW_SKEW_RATIO 1.5); position-detail `fetchRecentFlows` |
| HEATMAP ? LARGO | PASS | `get_gex` reads desk.gex_walls (SPX) / polygon GEX (per-ticker); SPY-derived caveat noted |
| HEATMAP ? NHAWK | PASS | nighthawk/positioning.ts imports shared `getGexPositioning`; LIVE thesis cites GEX call wall |
| HEATMAP ? NWATCH | PASS | verdict.ts reads shared `gexWalls` (spx-desk OR gex-heatmap); WALL_APPROACH/BREAK logic; getNwTickerGex |
| LARGO ? SPX/GEX/HELIX | PASS | 90+ tools incl get_spx_*, get_gex, get_options_flow |
| LARGO ? NWATCH | PASS | `get_my_positions` / `get_positioning` via getEnrichedPositionsForUser |
| LARGO ? NHAWK | PASS | `get_nighthawk_edition` / `get_nighthawk_outcomes` / `get_nighthawk_dossier` |
| LARGO ? GRID | PASS | `get_news` / `get_economic_calendar` / `get_dark_pool` / `get_earnings` / `get_congress_trades` / `get_catalysts` |
| GRID ? SPX | PASS | SPX desk carries macro_events=4 (Job openings, Consumer confidence, Chicago PMI, Case-Shiller), news_headlines=10, macro_indicators (GDP/CPI/UNRATE) |
| GRID ? NHAWK | PASS | data-sources.ts pulls news/earnings/FDA/congress catalysts into hunt |
| NHAWK ? LARGO/NWATCH | PASS | edition + dossier reachable via Largo tools; positioning shared |

### Data Timestamps (UTC)
- SPX desk as_of: 2026-06-30T09:56:46Z (fresh)
- GEX asof: 2026-06-30T09:56:04Z (gex_age_ms ~41s, gex_stale=false)
- HELIX flows: ~13.8h old (flow_data_age_ms 49.65M) � **expected**: market closed since 16:00 ET yesterday, now pre-market. feed_stalled=false.
- Night Hawk published_at: 2026-06-30T03:38:13Z (overnight hunt), carry_until_close=true

### Notes / SKILL calibration
- **No real FAILs.** The SKILL's naive SPX-price-vs-GEX-spot diff (6698) is a FALSE alarm: SPX desk is index-scaled (~7440) and the GEX endpoint is SPY-scaled (~742) � a 10� ratio, by design. The GEX provider's own cross-validation confirms wall/flip agreement.
- SKILL's HELIX?SPX field check (`flowBias`/`netFlow`/`flowSignal`) uses non-existent field names; real fields are `flow_0dte_net` / `tide_bias` / `spx_flows`.
