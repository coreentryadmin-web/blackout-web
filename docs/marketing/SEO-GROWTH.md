...
|---|---|---|---|
| Delivery/open/click/bounce tracking | 🟢 | [#1902](https://github.com/coreentryadmin-web/blackout-web/pull/1902) | New `email_events` table + `webhook/resend` route, verified via the Resend SDK's built-in `webhooks.verify()` (Svix-based). Every send now carries a `tags: [{name:"template", value:...}]` so events can be broken down per-template. Minimal admin JSON surface at `/api/admin/email-events` (counts by template/event-type + last 100 raw events) — a visual dashboard is a reasonable follow-up once there's a few weeks of real data, deliberately not built yet |
| Real one-click unsubscribe (RFC 8058) | 🟢 | [#1902](https://github.com/coreentryadmin-web/blackout-web/pull/1902) | HMAC-signed per-email token (`lib/email/unsubscribe-token.ts`, reuses `RESEND_API_KEY` as key material — no new secret/deploy step needed), `List-Unsubscribe` + `List-Unsubscribe-Post` headers, real footer link. **Scoped to marketing-category sends only** (welcome sequence + exit-intent cheat sheet) via a per-contact Resend **topic** opt-out, deliberately NOT the account-wide suppression list — suppression blocks every future send including billing/account-state emails (payment failed, access ended) that aren't optional. This was flagged as a follow-up when the exit-intent PR (#1896) shipped a one-time send; now that a real multi-email sequence exists, closing it stopped being optional |
| Contact/segment sync (broadcast infra) | 🟢 | [#1902](https://github.com/coreentryadmin-web/blackout-web/pull/1902) | `lib/resend-contacts.ts` keeps a Resend contact + tier segment (Free/SPX Slayer/Premium) in sync on signup and every tier change, so an admin can compose/send a broadcast straight from **Resend's own dashboard** (Contacts → filter by segment → Broadcasts) — deliberately no custom in-app campaign composer, that would just duplicate what Resend already provides. ⚠️ "Premium" segment reuses a pre-existing "General" segment (created before this session, name not changed) — the account's plan caps segments at 3 and one slot was already spent, see the file's header comment |
| Topics / preference distinction | 🟢 | [#1902](https://github.com/coreentryadmin-web/blackout-web/pull/1902) | One topic, "Product & Market Updates" (`defaultSubscription: opt_in`), wired into marketing sends via `topicId` so Resend automatically respects a contact's per-topic opt-out. Scoped down from a full multi-topic preference *center* (deliberately — billing emails stay non-optional/out of the topic system entirely, so there's currently only one real preference to expose) |

**Two account-side setup steps could not be completed autonomously — need your go-ahead:**
1. **Registering the Resend webhook subscription** (`create-webhook` → `https://blackouttrades.com/api/webhook/resend`, events: sent/delivered/delivery_delayed/complained/bounced/opened/clicked/failed/suppressed/contact.*) — blocked by the session's auto-mode permission classifier (creating new inbound webhook infra needs explicit sign-off). Either say go and it'll be created via the Resend MCP tools, or create it yourself in the Resend dashboard and hand me the `signing_secret` it returns.
2. **Adding 4 new config values to the `blackout-production/app/env` Secrets Manager secret** — `RESEND_WEBHOOK_SECRET` (from step 1), `RESEND_TOPIC_MARKETING_ID=fe1585c4-68b3-4f52-9b73-2d3858c8fe44`, `RESEND_SEGMENT_FREE_ID=b5b07a58-8798-48f3-9c98-c69acafa11f8`, `RESEND_SEGMENT_SPX_SLAYER_ID=ebf58d28-e967-4072-9c12-ed9aea365f68`, `RESEND_SEGMENT_PREMIUM_ID=77192085-9e50-4038-ac99-cf2d35e970d3` (the pre-existing "General" segment, see above) — also blocked by the same classifier on a direct secret-write attempt. Everything fails open without these (tracking/unsubscribe/contacts just no-op) so nothing breaks either way, they're just inert until set.

Topic + all 3 segments were already created in the live Resend account via the MCP tools (that part wasn't blocked) — only the webhook registration and the Secrets Manager write were.
