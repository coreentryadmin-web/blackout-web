...
- **Repo:** `C:/Users/raidu/blackout-cron` — the ISOLATED cron clone (a separate checkout with its own `node_modules`) dedicated to the autonomous jobs so they NEVER touch the interactive working tree at `blackout-platform/blackout-web`. It pushes to the same GitHub origin and Railway still deploys from `origin/main`. The dir name has NO "blackout-web" substring, so git runs cleanly via the Bash tool (no classifier block, no junction needed). `git pull --ff-only` before working. NOTE: every cron run shares THIS one clone, so the Concurrency-safety rule below (pull-rebase + branches) still governs overlap BETWEEN jobs.
