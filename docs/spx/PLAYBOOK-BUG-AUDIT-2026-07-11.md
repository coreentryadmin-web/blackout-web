...
## Claude — fifth-pass validation (2026-07-11, PRs #98/#99, staging live-checked)

Method: 4 independent agents adversarially re-verifying #98 (F3 governor thread, #20b admin routes, Q4 verdict-guard assert) and #99 (GHA fail-signal correctness — the primary ask this pass). Plus direct local runs: `tsc --noEmit` clean, `node --test "src/features/spx/**/*.test.ts"` → **420/420 pass**. Live staging cross-check this pass was partial: `/api/market/spx/desk` still shows `vwap_volume_weighted: false` (confirmed), but `/api/market/spx/play` returned `"available":false, "Session closed"` with no `playbook_shadow` block at all (deeper off-hours than the fourth pass caught) — so PB-01/PB-02 verdict text could **not** be re-confirmed live this pass; treat that specific line item as carried forward from the fourth pass, not re-proven. The two new admin routes (`fsm-today`, `promotion-report`) use genuine `requireAdminApi()` Clerk/Cognito auth, not the `CRON_SECRET` bearer — this sandbox has no AWS CLI configured to mint that session, so both were verified by code + local test only, not hit live. Read-only, no files edited.

### #99 — GHA fail signal: correctly scoped, one brittleness note

Confirmed no `continue-on-error` anywhere in the workflow; a hard crash or missing DB now genuinely fails the step (`PLAYBOOK_EVIDENCE_REQUIRE_DB=1`). Traced `assessPlaybookEvidenceAlerts` in full: it only ever produces a `fail`-level alert for `data_quality_session_coverage` failing on an **allowlisted** playbook (PB-01/02/03); non-allowlisted playbooks are skipped unconditionally; a low-sample `insufficient` tier alone (the normal, expected state for most playbooks for a long time) produces at most a `warn`, never a fail. Walked a concrete realistic scenario (PB-01 with only 4 triggers this week) through the actual code — confirmed it passes (warn only), not a false-positive fail. Discord notify is correctly optional and only fires when there's an actual alert. **This closes the primary question this pass cleanly.**

Two things worth tracking, neither blocking: (1) no test proves the *negative* — that insufficient-tier-without-data-quality-failure does NOT produce a fail alert, and that a non-allowlisted playbook is excluded even with bad data quality — this is currently only verified by manual code tracing (this review), not a regression test; (2) a genuine brittleness risk, not a bug: at low session counts (early in data collection), a single flaky/stale snapshot on one session can drag `data_quality_session_fraction` below the 0.95 threshold and hard-fail the GHA run — arguably correct behavior (it's a real data-quality event), but will likely read as noisy to an operator during the first few weeks. Also: the evidence script's allowlist (`["PB-01","PB-02","PB-03"]`) is a separate hardcoded literal, not imported from the canonical `PLAYBOOK_LIVE_ALLOWLIST_DEFAULT_STAGING` — currently in sync, but could silently drift if the live allowlist ever changes via env override.

### #98 — F3 governor single-thread: genuinely fixed, stronger guarantee than expected

Confirmed exactly one call to `evaluateTradeGovernor` now exists in the whole `src/features/spx` tree. The new `mergeTradeGovernorWithOptionOverlay` doesn't re-run session-level logic at all — it's a pure post-processing step over the already-computed governor result, and its input type is structurally limited to option-ticket fields only (`{mid, spread_pct, blocked, block_reason}`), with no session/desk/direction data in scope. This is a materially stronger fix than PR #88's "synchronize the two calls' bypass formula" — a future engineer adding a new session-level check now has nowhere to accidentally duplicate it, because the overlay function's type signature can't reach session data at all. Tests directly prove additive (not replacement) composition: a base result with a cooldown bypass plus an overlay-added spread block correctly shows both applied without either erasing the other. All 28 relevant tests (governor + engine + gates) pass, no regressions found. **The F3 fragility flagged in the third-pass review is closed, not just narrowed.**

### #98 — #20b admin routes: real, but the data-quality gate under-covers most playbooks

`fetchPlaybookPromotionEvidenceRows` genuinely adds the two previously-missing pieces (trigger-time feature snapshot, `option_ticket`/execution-sim data) via correctly-shaped subqueries, cross-checked against the real schema. The admin route (`/api/admin/playbook/promotion-report`) genuinely runs `evaluatePlaybookPromotion`/`evaluatePromotionGates` on real rows, not raw data — this is a genuine, on-demand, single-query production caller matching the `admin/bie-report` pattern. `/api/admin/playbook/fsm-today` reads real, already-existing FSM columns and would show genuine armed/triggered/open timestamps if queried during RTH.

**One real gap found**: the new `sessionSnapshotDataQualityOk()` data-quality check is a standalone reimplementation that only checks `desk_stale` and `vwap_volume_weighted` (PB-01/02-specific) — it does **not** call the existing, already-tested `evaluatePlaybookDataSatisfaction`, and so ignores `halt_channel_stale`, `gex_missing`, and `vix` entirely. Per `playbook-data-requirements.ts`'s own `REQUIREMENTS_BY_PB`, most of the other 12 playbooks require GEX/VIX/halt-feed freshness that this gate never checks — so promotion decisions for those playbooks currently get zero data-quality protection from this mechanism, even though the underlying data exists and the checking function already exists. `playbook-promotion-eval.ts`'s own code comment ("evaluatePlaybookDataSatisfaction not wired — no production callers") is **still literally true** after this PR. Small, contained fix: swap the standalone reimplementation for a call to the existing function, or extend it to check `REQUIREMENTS_BY_PB` per playbook. Test coverage is also narrow — the 9 new tests cover only the fraction/alert math, not `buildPromotionSample`'s market-condition-bucket mapping or `summarizePlaybookEvidence`'s win-rate/expectancy math, which is the actual "hoisted sample-builder" logic the PR is centered on.

### #98 — Q4 verdict-guard assert: does not match the recommended design, ships inert

This is the most substantive finding this pass. The recommendation was an independent check that `prev` FSM state is genuinely `armed` (or non-idle) at the final trigger gate, specifically to catch a **stale in-memory counter vs. a persisted state that's since reset to idle**. What shipped instead:
1. **The frozen-state check never flags `idle`** — only terminal/post-entry states are excluded — so the exact desync scenario the recommendation named (counter stale-high, state reset to idle) sails through both checks untouched.
2. **It re-derives `prev`/`armedPolls` from the same `snapshots`/`armedPollCounts` inputs the guard itself already used** to produce its result — there's no independent source of truth here, so structurally it can only catch a bug in the guard's own internal arithmetic, not a desync between two different code paths or ticks. This is a self-consistency postcondition, not the cross-source defense-in-depth that was asked for.
3. **The one new test is a no-op**: it constructs a case where `trigger_fired` is already `false` before the assertion function's first check even runs, so none of the three assertion branches (precondition, frozen-state, armed-polls) are ever actually exercised. Zero genuine pass case and zero genuine violation case are tested.
4. **It's completely unwired operationally** — `PLAYBOOK_VERDICT_GUARD_ASSERT` appears nowhere outside the source file itself and one "done" checkbox in this doc: not in any CI workflow, validation script, or environment config. There's no plan for it to ever actually run. It also isn't logged in `docs/audit/FINDINGS.md`, which this repo's own standing policy requires for every fixed issue.

Recommend re-opening this specific item: rename/reframe as a self-consistency check (accurate description of what it does today) if that's sufficient, or rebuild it to independently re-query persisted instance state (not reuse the guard's own inputs) and explicitly include `idle` in the frozen check, if the original cross-source-desync concern is still considered worth guarding against.

### Status update — foundation closure checklist

1. **F1 (VWAP)** — ✅ closed, reconfirmed this pass at the code level (live re-confirmation pending next market-open window).
2. **F2 (promotion pipeline runs against real data)** — ✅ closed for the scheduling/wiring goal; 🔧 the data-quality dimension of it is narrower than intended (see #20b finding above).
3. **F3 (dual-evaluation)** — ✅ closed. The governor fix is the strongest of the fixes verified this pass.
4. **F4 (RTH proof)** — still open, unchanged from the third/fourth pass; `fsm-today` is now a real, ready tool for it (confirmed genuine this pass), but no RTH session has actually been observed yet — market was closed for this entire review.
5. **F5 (dedup ranking)** — still valid as ranked in the third pass; #98's governor fix doesn't change the ranking (it was a dual-evaluation fix, not a god-file/status-system/window-duplication fix), and no new information this pass changes the relative ordering of #33/#9/temporal-windows/#13.
