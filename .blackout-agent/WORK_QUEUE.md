...
**Standing tasks** (`QUEUED (standing)`) are perpetual — never mark DONE. `select-task` also auto-discovers open PRs and deploy drift when no explicit tasks are queued.
