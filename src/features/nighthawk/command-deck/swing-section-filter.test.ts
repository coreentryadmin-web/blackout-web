...
// Section filter bar removed 2026-09-05 — Swing Command uses the same OPEN/WATCH/CLOSED bar as 0DTE.
test("containers: no seven-section filter bar (0DTE-style status filter only)", async () => {
  const { readFileSync } = await import("node:fs");
  const { fileURLToPath } = await import("node:url");
  const src = readFileSync(fileURLToPath(new URL("./containers.tsx", import.meta.url)), "utf8");
  assert.equal(src.includes('aria-label="Filter swing plays by serving section"'), false);
  assert.equal(src.includes("nh-deck-filterbar--sections"), false);
});
