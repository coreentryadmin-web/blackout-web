...
export function LegacyDeck({ edition }: { edition: NightHawkEdition | undefined }) {
  ...
  const { data: confirmData } = useSWR(
    ...
    () => fetch(`/api/nighthawk/play-status?edition_for=${editionFor}`, { cache: "no-store", credentials: "same-origin" }).then((r) => r.ok ? r.json() : null),
  ...
  const plays: TerminalPlay[] = (edition?.plays ?? []).slice(0, 5).map((p, i) => {
    ...
    return terminalPlayFromEdition({
      ...
      conviction: (p as { conviction?: string }).conviction ?? null,
      thesis: (p as { thesis?: string }).thesis ?? null,
      key_signal: (p as { key_signal?: string }).key_signal ?? null,
      entry_range: (p as { entry_range?: string }).entry_range ?? null,
      target: (p as { target?: string }).target ?? null,
      stop: (p as { stop?: string }).stop ?? null,
      options_play: (p as { options_play?: string }).options_play ?? null,
      entry_premium: (p as { entry_premium?: number }).entry_premium ?? null,
      risk_note: (p as { risk_note?: string }).risk_note ?? null,
      exit_style: (p as { exit_style?: string }).exit_style ?? null,
      iv_rank: (p as { iv_rank?: number }).iv_rank ?? null,
      rr_ratio: (p as { rr_ratio?: number }).rr_ratio ?? null,
      gate_promoted: (p as { gate_promoted?: boolean }).gate_promoted ?? null,
      gate_warnings: (p as { gate_warnings?: string[] }).gate_warnings ?? null,
      pulled: (p as { pulled?: boolean }).pulled ?? null,
      pulled_reason: (p as { pulled_reason?: string }).pulled_reason ?? null,
