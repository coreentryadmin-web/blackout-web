 ...
 * production-log dig. Stages, left→right, mirror the pipeline order:
...
export async function buildEveningEdition(opts?: {
  ...
  if (job?.status === "published" && opts?.force) {
    ...
    const inWindow = isInEditionWindow();
    const priorPublished = job.published_at;

    if (
      shouldRebuildStalePublishedEdition({
        publishedAtIso: job.published_at,
        windowStartMinutes: editionWindowStartMinutes(),
        inEditionWindow: inWindow,
      })
    ) {
      console.info(
        `[nighthawk/edition] stale edition detected — published ${priorPublished ?? "missing"} ` +
          `before today's window (${todayEt()} ${Math.floor(editionWindowStartMinutes() / 60)}:${String(editionWindowStartMinutes() % 60).padStart(2, "0")} ET). ` +
          `Rebuilding with fresh post-close data.`
      );
      await archiveAndClearNighthawkStaging(editionFor);
      await upsertNighthawkJob(editionFor, {
        status: "running",
      ...
      logNighthawkJob(
        ...
        "info",
        null,
        `Stale-edition rebuild — prior published ${priorPublished ?? "missing"} before window`
      );
    }
  ...
  try {
    ...
    if (checkpointedSynthesis && Array.isArray(checkpointedSynthesis.plays) && checkpointedSynthesis.plays.length) {
      ...
      if (!finalPlays.length && criticRescueEnabled()) {
