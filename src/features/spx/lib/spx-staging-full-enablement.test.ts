...
// STAGING FEATURE GATES (decommissioned 2026-07-25, but testing remaining live gates):
// Staging environment is deleted; isStagingDeploy() always returns false. However, some SPX gates
// remain as deliberate product choices (widening on staging, conservative on prod). These tests
// exercise those remaining gates (regime eligibility, watch visibility) which await product decision
// before finalization. Allowlist tests removed (2026-08-23) since that gate was entirely cleaned up.
...
// ── 1) REGIME ELIGIBILITY: staging makes the whole set eligible; prod filters by bucket ──────────
...
// ── 2) VWAP PROXY: merging proxy volume yields a TRUE volume-weighted VWAP ───────────────────────
...
// ── 3) WATCH VISIBILITY: a sub-threshold SCANNING candidate surfaces as an honest low-conviction chip
const scanningPlay = {
  available: true,
  phase: "SCANNING",
  action: "SCANNING",
