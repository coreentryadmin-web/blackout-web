 ...
 * (Voyage embeddings + cosine match) instead of Anthropic. No Largo/Claude on
 * this path; staging never calls this with Anthropic either.
 */
export async function evaluateClaudePlayApproval(
  ...
  const requireBie = playClaudeGateEnabled() || options?.forceClaude === true;
