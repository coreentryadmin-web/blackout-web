...
type Projected = {
  rightX: number;
  /** Left edge of the profile band — POC/VAH/VAL labels anchor here to avoid axis-badge collisions. */
  labelX: number;
  ...
  draw(target: PaneRendererTarget): void {
    target.useMediaCoordinateSpace((scope) => {
      ...
      for (const lvl of levels) {
        ...
        // Anchor at the profile band's left edge — axis price-line badges sit at rightX and
        // previously painted over right-aligned POC/VAH/VAL labels when levels were close.
        ctx.textAlign = "left";
...
export class VolumeProfilePrimitive implements ISeriesPrimitive<Time> {
  ...
  project(): Projected | null {
    ...
    return { rightX: gutter.rightX, labelX: gutter.gutterLeft + 4, bars, levels };
  }
}
