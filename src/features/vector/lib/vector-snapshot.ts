...
export async function recordVectorWallSamplesFromWarm(ticker: string): Promise<boolean> {
  ...
  const gexRecordable =
    walls != null && isWsUpdatedAtFresh(s.cachedWallsAt, STALE_RECORD_MAX_MS, nowMs);
  const vexRecordable =
    vexWalls != null && isWsUpdatedAtFresh(s.cachedVexWallsAt, STALE_RECORD_MAX_MS, nowMs);
...
export async function buildVectorStreamPayload(
  ...
  const gammaFlip = s.cachedFlip;
  const hubNowMs = Date.now();
  if (!isWsUpdatedAtFresh(s.cachedFlipAt, FLIP_CACHE_MS, hubNowMs) && !s.flipRefreshInFlight) {
