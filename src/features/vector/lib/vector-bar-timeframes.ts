...
export function wallCountForTimeframe(tf: VectorTimeframeMinutes): number {
  ...
  // Restored 2026-09-05: member screenshots (Sep 3 desk) show the pre-#2341 row ladder — denser
  // horizontal bead ribbons at SPX 3m. #2341 walked counts down to fix thickness by shrinking
  // rows; that traded away the Skylit-style bead structure members asked to keep.
  if (tf <= 1) count = 10;
