...
describe("suggested plays from thermal data", () => {
  const baseThermal: MeridianEarningsThermalRead = {
    available: true,
    spot: 500,
    call_wall: 505,
    put_wall: 495,
  ...
  test("returns null when walls are missing", () => {
    const thermal: MeridianEarningsThermalRead = { ...baseThermal, call_wall: null };
  ...
  test("generates play with call-wall-heavy regime (puts primary)", () => {
    ...
    const play = suggestedPlayFromThermal(baseThermal, 2.5, "2026-09-02"); // Wednesday
    assert.ok(play);
  ...
  test("generates play with put-wall-heavy regime (calls primary)", () => {
    ...
    const thermal: MeridianEarningsThermalRead = {
      ...baseThermal,
      call_wall: 490,
      put_wall: 510,
    };
    const play = suggestedPlayFromThermal(thermal, 2.5, "2026-09-02");
    assert.ok(play);
  ...
  test("calculates positioning_pct from wall ratio", () => {
    ...
    const play = suggestedPlayFromThermal(baseThermal, 2.5, "2026-09-02");
    assert.ok(play);
    assert.equal(play.positioning_pct, 51);
  });

  test("selects expiry as next Friday", () => {
    // 2026-09-02 is Wednesday → next Friday is 2026-09-04
    const play = suggestedPlayFromThermal(baseThermal, 2.5, "2026-09-02");
    assert.ok(play);
    assert.equal(play.expiry, "2026-09-04");
  });

  test("labels regime as 'balanced' when walls are within 5% of each other", () => {
    // Walls 502.5 and 497.5 → ratio ~1.01, within 1.5% of 1.0
    const thermal: MeridianEarningsThermalRead = {
      ...baseThermal,
      call_wall: 502.5,
      put_wall: 497.5,
