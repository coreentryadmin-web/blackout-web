...
test("rowsNeedingExpectedMoveOverlay excludes printed rows from the EM chain budget", () => {
  const rows = [
    {
      ticker: "NVDA",
      name: "NVIDIA",
  ...
  const need = rowsNeedingExpectedMoveOverlay(rows);
  assert.equal(need.length, 1);
  assert.equal(need[0]?.ticker, "AAPL");
});
