...
test("decodeClerkSessionJwtPayload: adds base64 padding for atob", () => {
  // Payload length mod 4 === 2 — needs "==" padding without the helper fix.
  const token = jwt({ sub: "user_pad", exp: 9_999_999_999, sts: "active" });
  assert.equal(decodeClerkSessionJwtPayload(token)?.sub, "user_pad");
});
