...
test("live Discord digest crons are on the health board", () => {
  const keys = new Set(CRON_JOBS.map((j) => j.key));
  for (const key of ["darkpool-discord", "helix-discord-digest", "thermal-discord"] as const) {
    assert.ok(keys.has(key), `${key} must be registered for staleness monitoring`);
    const job = CRON_JOBS.find((j) => j.key === key)!;
    assert.equal(job.produces_member_alert, true);
