 ...
 *
 * Per-replica Map is fine: each ECS replica caches independently and the TTL keeps
...
function setTierCache(userId: string, tier: Tier): void {
  ...
  if (tierCache.size >= MAX_TIER_CACHE) {
    ...
    for (const [k, v] of Array.from(tierCache)) {
      if (!isWsUpdatedAtFresh(v.at, TIER_CACHE_TTL_MS, now)) tierCache.delete(k); // reclaim expired before evicting live keys
    }
