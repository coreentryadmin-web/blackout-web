...
export function derivePlayStatus(input: {
  ...
  /** Sold iron condor: credit received, mark is debit-to-close — skip directional TRIM/stop. */
  isCondor?: boolean;
}): LivePlayState {
  const { entryPremium, mark, peak, trough, nowEtMinutes, deferPlanStop } = input;
  const isCondor = input.isCondor === true;
  ...
  const pnl =
    entryPremium != null && entryPremium > 0 && mark != null && mark > 0
      ? Math.round(
          (isCondor
            ? (entryPremium - mark) / entryPremium
            : (mark - entryPremium) / entryPremium) * 10000
        ) / 100
      : null;
  ...
  // Condors are held to the hard time-stop; breach/win is owned by gradeCondorFromBars.
  // Directional peak/trough latch semantics invert for credit structures (a falling mark is
  // winning) — never emit TRIM or CLOSED("stopped") off PLAN_RULES thresholds.
  if (isCondor) {
