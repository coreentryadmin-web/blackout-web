...
// plus one defensive structural guard (a dominant side with premium but somehow no
// strike on record, practically unreachable in real data) — and `continue`s straight
// past any candidate that fails one, with nothing surviving past that loop iteration.
...
export type ZeroDteGateRejection = {
   ...
   *  no_dominant_strike guard, which has no numeric threshold. */
  ...
  /** Null when the scan never reached the moneyness gate (or underlying price was
   *  unknown, in which case the live scan never checks this gate at all either). */
  otm_pct: number | null;
