...
function readUwCache<T>(key: string, allowStale: boolean): T | undefined {
  ...
  const now = Date.now();
  const age = now - slot.fetchedAt;
  // Clock-skewed future fetchedAt makes age negative → falsely inside TTL forever.
  ...
  if (effectiveAge <= ttl) return slot.data as T;
  if (allowStale && effectiveAge <= UW_SLOW_CACHE_MAX_STALE_MS) return slot.data as T;
  return undefined;
}
  ...
  for (const [name, fn] of attempts) {
    try {
      ...
      if (rows.length) {
        ...
        // warn made a healthy per-cycle build read as a red error in production logs (finding #76). The
