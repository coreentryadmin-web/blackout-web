...
// ----------------------------- snapshot valuation == chain valuation -----------------------------

test("valuationFromSnapshot equals valuationFromContract for the same Massive fields", async () => {
  const { valuationFromSnapshot, valuationFromContract } = await import(
    "@/lib/nights-watch/valuation"
  );
  const { mapUnifiedSnapshotResult } = await import("./options-snapshot");

  // One Massive result drives BOTH paths: the snapshot (mapped) and the chain (ChainContract
  // reads the same last_quote/greeks/iv/oi/underlying), so by construction they must match.
  const bid = 10.0;
  const ask = 10.4; // mid = 10.2 == midpoint
  const result = {
    ticker: "O:SPXW250620C05850000",
    type: "options",
    implied_volatility: 0.21,
    open_interest: 1234,
    greeks: { delta: 0.55, gamma: 0.01, theta: -0.4, vega: 1.2 },
    last_quote: { bid, ask, midpoint: 10.2 },
    last_trade: { price: 10.1 },
    details: { strike_price: 5850, contract_type: "call", expiration_date: "2025-06-20" },
    underlying_asset: { price: 5872.5 },
  };
  const snap = mapUnifiedSnapshotResult(result);
  assert.ok(snap);

  // Equivalent ChainContract built from the SAME fields (chain uses mid(bid,ask) = 10.2).
  const chainContract = {
    details: { strike_price: 5850, contract_type: "call", expiration_date: "2025-06-20" },
    greeks: { delta: 0.55, gamma: 0.01, theta: -0.4, vega: 1.2 },
    implied_volatility: 0.21,
  ...
  const fromSnap = valuationFromSnapshot(snap!, null);
  const fromChain = valuationFromContract(chainContract, 5872.5, null);
  assert.ok(fromSnap);
  assert.ok(fromChain);
  // Identical numbers (the whole point — a fetch optimization, not a math change).
  assert.equal(fromSnap!.mark, fromChain!.mark);
  assert.equal(fromSnap!.delta, fromChain!.delta);
  assert.equal(fromSnap!.gamma, fromChain!.gamma);
  assert.equal(fromSnap!.theta, fromChain!.theta);
  assert.equal(fromSnap!.iv, fromChain!.iv);
  assert.equal(fromSnap!.openInterest, fromChain!.openInterest);
  assert.equal(fromSnap!.underlyingPrice, fromChain!.underlyingPrice);
  assert.equal(fromSnap!.mark_source, "snapshot");
  assert.equal(fromChain!.mark_source, "snapshot");
});

test("snapshot == chain when only the day/session close is usable (C1: day-close tier)", async () => {
  const { valuationFromSnapshot, valuationFromContract } = await import(
    "@/lib/nights-watch/valuation"
  );
  const { mapUnifiedSnapshotResult } = await import("./options-snapshot");

  // No usable quote (ask 0) and no trade — only the session/day close. Before the fix the
  // snapshot lacked this tier and returned null here while the chain valued via day.close (C1).
  const result = {
    ticker: "O:SPXW250620P05000000",
    type: "options",
    implied_volatility: 0.3,
    open_interest: 50,
    greeks: { delta: -0.1, gamma: 0.02, theta: -0.2, vega: 0.8 },
    last_quote: { bid: 0, ask: 0 },
    last_trade: { price: 0 },
    details: { strike_price: 5000, contract_type: "put", expiration_date: "2025-06-20" },
    underlying_asset: { price: 5872.5 },
    session: { close: 3.25 },
  };
  const snap = mapUnifiedSnapshotResult(result);
  assert.ok(snap);
  assert.equal(snap!.dayClose, 3.25);
  assert.equal(snap!.mark, 3.25); // mid/last unusable → day close
  ...
  assert.ok(fromSnap);
  assert.ok(fromChain);
  assert.equal(fromSnap!.mark, 3.25);
  assert.equal(fromChain!.mark, 3.25);
  assert.equal(fromSnap!.mark, fromChain!.mark);
  assert.equal(fromSnap!.mark_source, "snapshot");
  assert.equal(fromChain!.mark_source, "snapshot");
});

test("valuationFromSnapshot: a live WS mark overrides the snapshot mark (source 'ws')", async () => {
  const { valuationFromSnapshot } = await import("@/lib/nights-watch/valuation");
  const snap = {
    ticker: "O:SPXW250620C05850000",
    mark: 10.2,
    bid: 10.0,
  ...
  const v = valuationFromSnapshot(snap, { mark: 11.0, bid: 10.9, ask: 11.1, ts: Date.now() });
  assert.ok(v);
  assert.equal(v!.mark, 11.0);
  assert.equal(v!.mark_source, "ws");
  // greeks still come from the snapshot (WS Q feed has none)
  assert.equal(v!.delta, 0.55);
});
