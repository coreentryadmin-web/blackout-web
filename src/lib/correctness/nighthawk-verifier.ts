    ...
    if (parseable.length === 0) {
      ...
      for (const { play, parsed } of parseable) {
        ...
        if (verdict.verified) {
          ...
          // Skip when the edition is stale — overnight gaps move option premiums far beyond
          // any reasonable band and produce false flags (NVDA evening publish vs RTH chain).
