/**
 * Clerk v7.5.x auth()/auth.protect() return null userId on /sign-in and /sign-up
 * even with a valid session cookie (authenticateRequest path inconsistency).
 * Middleware uses this decode-only fallback AFTER Clerk has already verified the
 * request — we never trust an unverified token for authorization on protected routes.
 */
...
export function activeClerkUserIdFromSessionCookie(
  ...
  // Clerk sets sts to "active", "pending", "ended", etc. — only redirect for active.
