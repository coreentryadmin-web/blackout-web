...
export function computeLitDarkRatio(): LitDarkRatio | null {
  const litFresh =
    litTradesStore.updatedAt > 0 && isWsUpdatedAtFresh(litTradesStore.updatedAt, LIT_DARK_MAX_AGE_MS);
  const darkFresh =
    darkPoolStore.updatedAt > 0 && isWsUpdatedAtFresh(darkPoolStore.updatedAt, LIT_DARK_MAX_AGE_MS);
