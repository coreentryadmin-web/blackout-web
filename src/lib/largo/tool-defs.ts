...
export const LARGO_TOOL_DEFS: AnthropicToolDef[] = [
  ...
  t(
    ...
    "HELIX velocity/split-flow signal follow-through tracker — graded vs pending signal outcomes from the helix_signal_outcomes ledger. Use for 'do HELIX velocity signals actually follow through'. Each row carries `fired_at` (raw ledger timestamp) AND `fired_session` (the ET calendar date it fired in) — read `fired_session`, never infer the session from `fired_at`, which is UTC and rolls to the next day after ~20:00 ET. The payload's own `session_date` is today's ET session. RATE — `summary.continuation_rate_pct` (same number as the legacy `winRatePct`) is a DIRECTIONAL FOLLOW-THROUGH rate, not a trade win rate and not a P&L. It is null below `summary.min_graded_for_rate` graded samples — null means NOT ENOUGH DATA, never 0%. NEVER report its complement as losses: the graded rows split three ways and the split is the answer. Read `summary.continuedCount` / `flatCount` / `reversedCount` — a signal that is 62% continued, 30% FLAT and only 7% REVERSED rarely goes wrong, it often goes nowhere, which is a completely different instrument from one that is wrong a third of the time. Say which. `outcome` is relative to each row's own `direction`, so a continued BEARISH firing means price FELL. `outcome_values` lists the vocabulary. `rows_shown` vs `rows_summarized` differ on purpose — the rate is computed over ALL fetched rows, the list is the newest few. TIME SCOPE — `summary.graded_window` names WHEN the rate is from: the graded rows' fired_at span as `oldest_session`/`newest_session` (ET) plus `span_hours`. The ledger holds the 50 most-recent rows and a fire is not graded until forward bars exist, so early in a session EVERY graded row is from a PRIOR session — `as_of` is the READ time, NOT the data's time. Quote the rate as \"X% over N fires between <oldest_session> and <newest_session>\", never as \"today\" unless `newest_session` IS today's `session_date`. PER TYPE — `summary.by_signal_type` breaks the same follow-through down per signal type (split_flow / velocity_spike / …), each with its OWN `graded` denominator and its OWN `continuation_rate_pct` (null below `min_graded_for_rate`, same rule as the aggregate). Use it for 'which HELIX signal is more reliable' — do NOT hand-count the capped `rows` list to compare types, it is a truncated slice and can rank them backwards. Each type carries its OWN `graded_window`, because split_flow and velocity_spike do not fire on the same cadence — one can be a session stale while the other is current. Per-type counts sum to the aggregate.",
  ...
  t(
    ...
    "ONE-CALL SPX desk convergence — Vector's suggested play (card bias/setup/grade) AND Slayer's execution layer (phase/action/gates/open play) plus live gate_rules, with an explicit alignment verdict (aligned / divergent / vector_leads / slayer_leads / flat). Includes `matrix_ui` — available GEX/VEX lenses, default GEX, and honest client-only notes for focus mode / iOS panel (server cannot read the member's active lens toggle). Includes `lane_freshness` — Pulse/Desk/Flow lane status chips matching the member header strip (live/stale/cached/offline per lane). Prefer this over calling get_vector_full_state(SPX) + get_spx_play separately when the member asks whether Vector and Slayer agree, what the suggested play is vs what the desk committed, or 'are we aligned on SPX'. Slayer execution (commits, open position, signal_committed) always wins for risk — Vector is the narrative suggestion.",
    {}
  ),
  t(
    "get_spx_voice_feed",
  ...
  t(
    ...
    "HELIX secondary panels computed from the Postgres flow tape — the aggregates the UI renders as Net Premium leaderboard, Route Breakdown (SWEEP/BLOCK/FLOOR/…/REPEAT), Expiry Concentration, and session call/put skew. get_helix_derived covers Stacked Hits/Velocity/Split Flow; THIS tool covers the leaderboard/route/expiry panels. Optional ticker narrows the tape to one name. SKEW vs DIRECTION — these are TWO DIFFERENT QUESTIONS and the payload answers them with two different fields; do not use one for the other. `session.call_pct` is the AUTHORITATIVE call/put premium SHARE for the whole session, computed server-side over the full member tape population. Use it for 'call vs put' and 'skew' questions — do NOT hand-sum get_flow_tape's capped print list, which is a slice and can read the OPPOSITE share (a 50-row premium-ordered pull read 34% call while this session read 60%). `call_pct` is NOT a direction. A bought call is bullish but a SOLD call is BEARISH, so a name can be 100% call premium and measurably bearish — live 2026-08-23, CG was exactly that at 100% readable, and so were DRAM, MSTR, BE, SMCI and MRVL. NEVER answer 'is the flow bullish/bearish' from `call_pct`; the member panels do not, and you would contradict what the member is looking at. For ANY bullish/bearish question read `direction` (bullish | bearish | mixed | undetermined), which applies the same aggression-aware rule the panels use (`direction_basis`, matching the signal ledger's stamp — never pool results across two bases). `direction` is present on `session`, on every `net_premium_leaders` row and on every `expiry_horizons` row. `undetermined` means REFUSED, not neutral — say so rather than falling back to `call_pct`. Read `direction_readable_pct` before quoting any direction: `ask_pct` is absent from the SPX/SPY index feed, which carries most premium, so a large population can be almost entirely unreadable (SPX measured 0.1% readable, LEAPS 3.2%, Monthly 6.1%). When `direction_minority_evidence` is true the verdict rests on a minority of the premium and you MUST state that share alongside it. `direction_readable_pct: null` means there was no premium at all, which is a different fact from 0% readable. `net` on a leaderboard row is calls minus puts — arithmetic about the share, NOT a direction; a large positive `net` beside a bearish or undetermined `direction` is correct and is often the most informative thing on the row, because it means the calls were being SOLD. EXPIRY — read `expiry_horizons` (0DTE / This week / Monthly / LEAPS, with call/put split) for any question about horizon, and ALWAYS for \"is there 0DTE flow\": it is the aggregation the member panel shows and it is complete. `expiry_concentration` is the per-DATE detail and is only the top 8 BY PREMIUM — `expiry_concentration_truncated` tells you whether dates were dropped, and near-dated expiries are the ones that drop, because 0DTE prints are small next to LEAPS blocks. Never infer a date's horizon yourself: every row carries `dte` and `horizon`, measured against `session_date` (the ET CALENDAR DATE this tape is scored against). `session_date` is NOT the same as the date part of `as_of` — after ~8pm ET the UTC date is already tomorrow, so treating an expiry as 0DTE because it matches `as_of` is wrong by a full session. `available` is true even on an empty tape; `empty_reason: \"no_prints_in_window\"` means the window was genuinely quiet, not that the tool failed. ROUTE — `route_breakdown` buckets UW's `alert_rule` by EXECUTION MECHANISM (SWEEP/BLOCK/SPLIT/CROSS/FLOOR/MULTI/GRENADE), then `REPEAT` for a repeated-hits rule that named no mechanism, then two buckets that are NOT routes and mean different things. `UNREPORTED` is a print carrying NO alert_rule at all, so no execution route was ever reported for it — say the route is unknown for those, never that it is 'other'. `OTHER` means a rule WAS present and named nothing we have a word for. UNREPORTED is routinely the LARGEST bucket (70% of the live tape on 2026-08-22) because one of the two feeds writing this tape sends no rule field at all, so a route breakdown describes the ~30% of prints that carry a rule — quote the bucket's own count rather than implying it characterises the whole tape. WINDOW — never quote `window.requested_hours` as the period you analysed. The row limit almost always binds before the time window does: a 168-hour request with limit 500 came back covering 54 MINUTES. Quote `window.actual_hours` (the span of the prints themselves) — or `window.actual_minutes` when the span is short, which it often is — and when `window.limit_reached` is true say the read is limit-bound, not window-bound. `window.no_dated_print_reason` distinguishes an empty window from one whose prints all lack an exchange timestamp. `window.newest_age_minutes` tells you how stale the freshest print is — off-hours a tape can be complete and still hours old. Rows are ordered NEWEST-FIRST, the same population the member's /flows desk shows; the desk additionally hides prints under `member_panel_premium_floor` ($200k) which this tool does NOT (`premium_floor_applied: false`). That cuts BOTH ways and neither is an error: this read includes small prints the desk hides, so small-print totals can be HIGHER here; but the desk's floored page reaches further back in time for the same row count, so a big name's total can be LOWER here than on screen. SKEW — `call_pct` is **null** whenever no call/put premium was measured (empty or all-typeless tape). null means NOT MEASURED; it does NOT mean balanced, and there is no 50 fallback. Say the tape is quiet rather than reporting an even split. `session.typeless_prints` reconciles `whale_prints` with `total_premium`: a typeless print counts as a print (and can be a whale) but adds to neither premium leg, so `{whale_prints: 1, total_premium: 0}` is coherent, not a broken sum. CONTRACT — `freshness` (live/delayed/stale) + `age_seconds` describe the newest REAL print, and both are **null when the tape's age cannot be measured at all** (most prints are ingest-stamped and carry no exchange time); null there means UNKNOWN, not stale. `direction` is present ONLY when the skew was measurable — its ABSENCE means not measured, and there is no neutral fallback. `ticker_class` (index/etf/equity) and `canonical_root` accompany `ticker`, which is left as the tape reported it: SPX and SPXW are different settlement series that both trade, so join on `canonical_root` but never restate SPXW as SPX. A genuine failure returns `available: false` with `unavailable: {reason, what_is_missing, retryable}` — distinct from a healthy but quiet tape, which stays `available: true` with `empty_reason`. Never report the second as the first.",
    {
  ...
  t("get_spx_confluence", "SPX confluence engine — the scored desk thesis: action (BUY_CALL/BUY_PUT/HOLD/WAIT), bias, score (-100..100), grade A+..D, agreeing vs conflicting factors with weights, entry/stop/target/invalidation. Explains WHY the desk leans a direction. Pure compute on the live desk. Carries NO `confidence`: SPX Slayer has no calibrated confidence model, so it is omitted rather than invented (`confidence_omitted` says so in the payload). Read conviction off `score`, `grade`, and `agreeing` vs `weighted_conflicts` instead."),

  t("get_positioning", "Dealer positioning for ANY ticker — full Thermal canonical read: spot, change_pct, net GEX/VEX/DEX/CHARM with posture + regime reads, gamma flip, call/put walls, gex king strike, max pain, nearest_wall, distance_to_flip_pct, AND regime-fragility fields — flip_crossings (how many times net gamma crosses zero), flip_nearest (the crossing NEAREST spot; equals flip when there is only one) and distance_to_nearest_flip_pct. When flip_crossings > 1 the reported flip is the deliberately STABLE lowest crossing and flip_nearest is the nearer one — cite THAT distance, not distance_to_flip_pct, when judging how close the name is to flipping regime. Plus intraday shift_summary and optional UW cross-validation. TIMING — read these before quoting a price: `as_of_et` is an ET wall-clock stamp and `session_date` is the ET SESSION this read belongs to; never derive a session from the UTC `asof`. `market_session` (OPEN / PRE-MARKET / AFTER-HOURS / CLOSED) is holiday-aware. `matrix_age_sec` is the age of the COMPUTATION, not of the price — a matrix rebuilt 60 seconds ago can be modelling a close that settled hours earlier — and `freshness` is 'cached' because this is a cache read. Outside OPEN, `spot` is the prior session's close or an extended-hours print, NOT a live quote: say which session a price belongs to rather than presenting it as current. Same object as get_ecosystem_context.gex_positioning. For SPX/I:SPX, strikes are SPX-denomination (thousands). Prefer get_gex_heatmap for per-strike ladder / lens toggles (GEX/VEX/DEX/CHARM).", T, ["ticker"]),

  t(
    "get_gex_heatmap",
    "BlackOut Thermal matrix for a ticker — the SAME shared gex-heatmap cache the /heatmap UI reads. Returns spot, change_pct, flip, walls, max pain, net GEX/VEX/DEX/CHARM, regime reads, nearest_wall, shift_summary, plus top_strikes ranked by |net| for the requested lens (gex/vex/dex/charm). When `chain_truncated` is true, the options-chain pagination guard exited before the full chain was read — walls and OI may be understated; say so explicitly instead of presenting levels as complete. Use for 'show me the gamma matrix', 'per-strike GEX on NVDA', 'VEX/DEX/CHARM lens', 'what is the king strike'. TIMING — `as_of_et` (ET wall-clock) and `session_date` (the ET SESSION) accompany the UTC `asof`; never derive a session from the UTC value. `market_session` is holiday-aware, and `matrix_age_sec` is the age of the COMPUTATION, not of the price it models — outside OPEN the `spot` is a close or an extended-hours print, not a live quote. `freshness` is 'cached' because this reads the shared matrix. NOT the same as get_gex (which reads SPX desk or raw Polygon 0DTE bundle).",
    {
      ...T,
