...
export const LARGO_TOOL_DEFS: AnthropicToolDef[] = [
  ...
  t("get_positioning", "Dealer positioning for ANY ticker — full Thermal canonical read: spot, change_pct, net GEX/VEX/DEX/CHARM with posture + regime reads, gamma flip, call/put walls, gex king strike, max pain, nearest_wall, distance_to_flip_pct, AND regime-fragility fields — flip_crossings (how many times net gamma crosses zero), flip_nearest (the crossing NEAREST spot; equals flip when there is only one) and distance_to_nearest_flip_pct. When flip_crossings > 1 the reported flip is the deliberately STABLE lowest crossing and flip_nearest is the nearer one — cite THAT distance, not distance_to_flip_pct, when judging how close the name is to flipping regime. Plus intraday shift_summary and optional UW cross-validation. Same object as get_ecosystem_context.gex_positioning. For SPX/I:SPX, strikes are SPX-denomination (thousands). Prefer get_gex_heatmap for per-strike ladder / lens toggles (GEX/VEX/DEX/CHARM).", T, ["ticker"]),
