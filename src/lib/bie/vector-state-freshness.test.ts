...
test("minor clock skew within tolerance clamps age to 0 and stays live", () => {
  const f = describeVectorFreshness(new Date(T0 + 3_000).toISOString(), T0);
  assert.equal(f.age_seconds, 0);
  assert.equal(f.freshness, "live");
});

test("clock skew beyond tolerance yields unknown, not live", () => {
