  ...
  for (const r of rows) {
    ...
    const res = await p.query<{ inserted: boolean }>(
      ...
      ON CONFLICT (session_date, ticker) DO UPDATE SET
        ...
        -- cycle that computes entry_premium (attachContractPlans -> buildOcc), so
        -- letting them update independently (the pre-fix behavior) let a later
        -- scan's shifted dominant strike silently pair with the ORIGINAL contract's
        -- entry_premium on the row/API response — a live, member-visible ledger
        -- row could show "top_strike: 730" next to an entry price that was actually
        -- for the 725 strike (see FINDINGS.md for the live case that surfaced this).
