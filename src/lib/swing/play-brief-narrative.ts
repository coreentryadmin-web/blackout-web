...
export function tradeManagerNarrativeSection(
  ...
  if (spot != null) {
    ...
    const vectorStaleForNarrative = vectorSnapshotStale(vec, Date.now());
    const prox = vectorStaleForNarrative ? null : vec?.proximity;
    if (prox?.callout && bullets.length < MAX_BULLETS) {
      add(`**Nearest wall ${prox.strike.toFixed(2)}** (${prox.side}) — ${prox.callout}`);
    }

    const walls = vectorStaleForNarrative ? [] : (vec?.wallEvents ?? []);
