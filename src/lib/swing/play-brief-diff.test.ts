...
test("diffBriefSnapshots: skips thesis health shift when pillars are uncalibrated", () => {
  const uncalibrated = {
    health: 48,
    rungLabel: "Degraded",
    pillars: [
  ...
  const prev = snapshotFromBrief(env(), play({ thesisHealth: { ...uncalibrated, health: 48 }, pnlPct: 20 }));
  const next = snapshotFromBrief(env(), play({ thesisHealth: { ...uncalibrated, health: 51 }, pnlPct: 25 }));
  const lines = diffBriefSnapshots(prev, next);
  assert.ok(!lines.some((l) => l.includes("Thesis")), "uncalibrated composite must not narrate thesis shift");
  assert.ok(lines.some((l) => l.includes("P&L")));
});
