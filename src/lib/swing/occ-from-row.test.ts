...
test("occSymbolFromSwingRow: rebuilds from strike/expiry/type when OCC missing", () => {
  assert.equal(
  ...
  assert.equal(
    occSymbolFromSwingRow({
      ...
      contract_expiry: "2026-08-15",
      contract_type: "put",
      contract_strike: 200.5,
    }),
    "O:AAPL260815P00200500",
  );
});

test("occSymbolFromSwingRow: null when identity incomplete", () => {
  assert.equal(occSymbolFromSwingRow({ ticker: "NVDA", contract_expiry: "2026-08-15" }), null);
  assert.equal(occSymbolFromSwingRow({}), null);
});

test("occFromChainContract: bare OCC for ledger (no O: prefix)", () => {
  assert.equal(
