/**
 * In-process RTH warm leader — backup when ECS/EventBridge cron trigger services stall.
 *
 * ECS/EventBridge cron triggers (hit-cron.mjs) can stop firing on schedule after redeploys
 ...
 * loop runs inside the always-on web server process, not an ECS-scheduled trigger, so it's
