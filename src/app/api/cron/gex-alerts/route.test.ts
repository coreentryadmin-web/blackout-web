...
test("gex-alerts gates on isEtCashRth before regime heatmap evaluation", () => {
  assert.match(
    routeSrc,
  ...
  const workAt = routeSrc.indexOf("for (const ticker of REGIME_WATCHLIST)");
  assert.ok(authAt >= 0 && gateAt >= 0 && workAt >= 0);
  assert.ok(gateAt > authAt, "RTH gate must run after auth");
  assert.ok(gateAt < workAt, "RTH gate must run before heatmap evaluation");
});
