 ...
 * SECOND run starting while the FIRST is still in flight — it is released the instant that run
 * completes. A caller replaying `?force=1` in a tight loop could re-trigger the scanner tick +
 * board snapshot rebuild far faster than any legitimate trigger (rth-warm-leader's 4 min heal
 * threshold or EventBridge's ~5 min schedule).
 *
