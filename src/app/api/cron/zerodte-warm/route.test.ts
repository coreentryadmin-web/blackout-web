...
test("force=1 is rate-limited by a minimum re-run cooldown, independent of the hours gate", () => {
  assert.match(routeSrc, /RERUN_COOLDOWN_SEC = 90/, "the floor must exist and be tuned below rth-warm-leader's 4min heal threshold");
  assert.match(routeSrc, /RERUN_COOLDOWN_KEY = "zerodte-warm:cooldown"/);
