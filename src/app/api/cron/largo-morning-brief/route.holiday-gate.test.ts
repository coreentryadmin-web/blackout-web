...
test("largo-morning-brief gates on isTradingDayEt before building the brief", () => {
  ...
  assert.ok(authAt >= 0 && gateAt >= 0 && windowAt >= 0 && buildAt >= 0);
  assert.ok(gateAt > authAt, "trading-day gate must run after auth");
  assert.ok(windowAt > gateAt, "ET window gate must run after trading-day gate");
  assert.ok(buildAt > windowAt, "brief build must run after ET window gate");
