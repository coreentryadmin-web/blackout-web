...
test("x-growth gates on isTradingDayEt before running the growth sweep", () => {
  assert.match(
    routeSrc,
  ...
  const sweepAt = routeSrc.indexOf("runGrowthSweep(");
  assert.ok(authAt >= 0 && gateAt >= 0 && sweepAt >= 0);
  assert.ok(gateAt > authAt, "trading-day gate must run after auth");
  assert.ok(sweepAt > gateAt, "growth sweep must run after trading-day gate");
