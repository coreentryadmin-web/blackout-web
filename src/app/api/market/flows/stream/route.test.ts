...
// /flows/stream is the live SSE lane for HELIX/Vector flow tape.
...
test("/flows/stream rounds floats before JSON.stringify, like /flows", () => {
  assert.match(sibling, /roundFloats\(payload\)/, "precondition: REST flows route rounds");
  assert.match(src, /import \{ roundFloats \} from "@\/lib\/round-floats"/);
  assert.match(src, /JSON\.stringify\(\s*roundFloats\(payload\)\)/);
});
