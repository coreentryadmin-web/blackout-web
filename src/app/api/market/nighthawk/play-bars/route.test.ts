...
const FAKE_BARS = [
  ...
  { t: Date.parse("2026-08-28T14:36:00.000Z"), o: 4.2, h: 4.5, l: 4.2, c: 4.400000000000001 },
...
describe("/api/market/nighthawk/play-bars", () => {
  ...
  test("rounds IEEE float noise on close prices at the response boundary", async () => {
    lastFetchArgs = null;
    const res = await GET(req("occ=O:NVDA260828C00190000&since=2026-08-28T14:35:00.000Z"));
    assert.equal(res.status, 200);
    const body = (await res.json()) as { points: Array<{ c: number }> };
    assert.equal(body.points[1]!.c, 4.4, "4.400000000000001 must round to 4.4 at the wire boundary");
  });
