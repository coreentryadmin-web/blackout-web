      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html:
              "try{if(/BlackOutiOSApp/.test(navigator.userAgent)){document.documentElement.classList.add('ios-app');var m=document.querySelector('meta[name=viewport]');if(m)m.setAttribute('content','width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover');var cw=Math.min(window.screen.width,window.innerWidth||window.screen.width);if(cw>=430){document.documentElement.classList.add('ios-tier-pro-max')}else if(cw>=393){document.documentElement.classList.add('ios-tier-pro')}var p=location.pathname;if(/^\\/(dashboard|flows|heatmap|terminal|nighthawk|grid|account|faq|learn|upgrade|admin)(\\/|$)/.test(p)){document.documentElement.classList.add('ios-app-pending-shell');if(p==='/dashboard'||p.indexOf('/dashboard/')===0)document.documentElement.setAttribute('data-ios-route','dashboard');else if(p.indexOf('/flows')===0)document.documentElement.setAttribute('data-ios-route','flows');else if(p.indexOf('/heatmap')===0)document.documentElement.setAttribute('data-ios-route','heatmap');else if(p.indexOf('/terminal')===0)document.documentElement.setAttribute('data-ios-route','largo');else if(p.indexOf('/nighthawk')===0)document.documentElement.setAttribute('data-ios-route','nighthawk');else if(p.indexOf('/account')===0)document.documentElement.setAttribute('data-ios-route','account')}}if(/BlackOutNativeEmbed/.test(navigator.userAgent)){document.documentElement.classList.add('ios-native-embed')}}catch(e){}",
          }}
        />
