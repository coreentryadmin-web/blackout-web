...
- `scripts/audit/discovery-recall-probe.mjs` — **discovery recall probe** (design Q10, "no silent caps"). Screens a session with the REAL production `screenBreakoutMovers` ranking, splits qualifying movers at the production cap (`BREAKOUT_MAX_CANDIDATES`=6) into KEPT(top-6) vs DROPPED(rank 7…N), and grades each name's intraday continuation on REAL Polygon minute bars (favorable-first long-call proxy) → per-cohort win-rate + the specific dropped winners the top-N cut never saw. Read-only, Polygon-only (no UW/DB/Clerk). Flags: `--grade=YYYY-MM-DD --scan-top=N --fav=0.015 --entry=10:00 --json`. Run with `env -u AWS_ACCESS_KEY_ID -u AWS_SECRET_ACCESS_KEY`. First 5-session run (2026-07-20…24): the dropped tail won ≥ the kept top-6 on 3/5 days, 10–17 winning movers dropped per session — the $-volume cap is leaky (see FINDINGS 2026-07-25).
